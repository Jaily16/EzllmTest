...
def get_integration_description(pid: str, integration_type: int, unit_name: str = ''):
    """获取集成description，并遵循现有调用契约。

    参数:
        `pid`：项目 ID。
        `integration_type`：沿用签名中 `int` 类型约束的输入。
        `unit_name`：沿用签名中 `str` 类型约束的输入。"""
    try:
...
def generate_integration_test_cases(integration_test_knowledge: str, strategy: str, strategy_knowledge: str,
    ...
    参数:
        `integration_test_knowledge`：沿用签名中 `str` 类型约束的输入。
        `strategy`：沿用签名中 `str` 类型约束的输入。
        `strategy_knowledge`：沿用签名中 `str` 类型约束的输入。
        `blackbox_method_knowledge`：沿用签名中 `str` 类型约束的输入。
        `integration_object`：沿用签名中 `str` 类型约束的输入。
        `integration_object_info`：沿用签名中 `str` 类型约束的输入。
