...
# 模块位于 backend/service/project，需上溯三层到后端根目录。
...
def _coerce_source_entry(
    ...
    参数:
        `entry`：沿用签名中 `SourceRevisionEntry | Mapping[str, Any]` 类型约束的输入。

    返回:
        `SourceRevisionEntry`，内容保持现有调用方契约。
...
def compute_source_revision(
    ...
    参数:
        `entries`：沿用签名中 `Iterable[SourceRevisionEntry | Mapping[str, Any]]` 类型约束的输入。
...
def artifact_input_hash(operation: str, payload: dict[str, Any]) -> str:
    ...
    异常:
        `ValueError, TypeError`：输入、状态或下游结果不满足现有约束时抛出。"""
    if not operation:
