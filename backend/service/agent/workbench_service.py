...
from service.agent.checkpoint import derive_agent_scope_hash
from service.agent.contracts import (
    AgentRunStatus,
    ApprovalDecision,
    TrustedProjectScope,
)
from service.agent.graph import graph_plan_hash
from service.agent.runtime_contracts import AGENT_GRAPH_VERSION
from service.agent.runtime import AgentRunHandle, AgentRuntimeService
...
class AgentWorkbenchService:
    ...
    async def create_run(
        ...
        异常:
            `AgentRunConflict`：输入、状态或下游结果不满足现有约束时抛出。"""
    ...
    async def decide_approval(
        ...
        异常:
            `ValueError, KeyError`：输入、状态或下游结果不满足现有约束时抛出。
    ...
    async def edit_run(
        ...
        """处理EDIT运行，并保持 `AgentWorkbenchService` 的现有状态约束。
    ...
    async def cancel_run(
        ...
        返回:
            `AgentRunView`，内容保持现有调用方契约。

        异常:
            `KeyError, AgentRunConflict`：输入、状态或下游结果不满足现有约束时抛出。"""
        view = await self.get_run(scope, thread_id)
        if view is None:
            raise KeyError("Agent thread was not found")
