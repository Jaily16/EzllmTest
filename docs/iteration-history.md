...
## 复现简化与本机试用（2026-09-11）

在 1efe60d267fd6b3d91f998948634ee2423214cfc 基线上增加显式 --local-config，按角色固定展开三端配置，保留完整路径模式；SQLite 模板使用配置目录相对路径，保留边界校验与绝对路径兼容。README 删除安装教程，保留版本表、必要凭据、Windows runner 主路径和 Linux 核心参考，并解释普通/不消费 worker、Uvicorn、MCP。性能采用“优化前/优化后（当前沿用）”，来源及历史测量条件不改写。

本次仅修改配置与启动便利能力、对应测试、示例说明及文档；本机配置和数据原地复用。本地说明 LOCAL-STARTUP.md 精确忽略，不上传。已完成离线检查及本机启动：三端和普通 worker 就绪，交给用户试用并保持运行。用户已明确“试用通过，可以发布”，发布阶段重新建立独立保护快照，复核 16 份待发布文件与试用字节记录一致。本提交用于向 main 与 iteration7 普通原子快进发布；最终提交与远程核验结果记录在发布批次 evidence，不预写提交自身 SHA。

试用中发现本机 Moonshot 密钥与国际站地址不匹配；只读模型列表检查确认国内站 HTTP 200 且包含 Kimi K3。经用户确认，仅切换本机端点并重启原 runner 进程组，其他配置保持。真实配置未纳入提交，助手没有主动发起生成请求。
