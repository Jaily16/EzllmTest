          ...
          >
            <el-sub-menu index="analysis">
              ...
              <el-menu-item-group title="规划工作流">
                <el-menu-item
                  index="/plan"
                  :disabled="!isWorkflowRouteAllowed('/plan')"
                ...
                <el-menu-item
                  index="/menu"
                  :disabled="!isWorkflowRouteAllowed('/menu')"
            ...
            <el-sub-menu index="manual">
              ...
              >
                <el-menu-item
                  index="/unit"
                  :disabled="!isWorkflowRouteAllowed('/unit')"
                ...
                >
                  ...
                  <el-tag
                    class="nav-state"
                    size="small"
                    :type="navigationTagType('/unit', 'unit_case')"
                    >{{ navigationStateLabel("/unit", "unit_case") }}</el-tag
                  >
                </el-menu-item>
                <el-menu-item
                  index="/integration"
                  :disabled="!isWorkflowRouteAllowed('/integration')"
                ...
                >
                  ...
                  <el-tag
                    class="nav-state"
                    size="small"
                    :type="navigationTagType('/integration', 'integration_case')"
                    >{{ navigationStateLabel("/integration", "integration_case") }}</el-tag
                  >
                </el-menu-item>
                <el-menu-item
                  index="/api"
                  :disabled="!isWorkflowRouteAllowed('/api')"
                ...
                >
                  ...
                  <el-tag
                    class="nav-state"
                    size="small"
                    :type="navigationTagType('/api', 'api_case')"
                    >{{ navigationStateLabel("/api", "api_case") }}</el-tag
                  >
                </el-menu-item>
                <el-menu-item
                  index="/ui"
                  :disabled="!isWorkflowRouteAllowed('/ui')"
                ...
                >
                  ...
                  <el-tag
                    class="nav-state"
                    size="small"
                    :type="navigationTagType('/ui', 'ui_case')"
                    >{{ navigationStateLabel("/ui", "ui_case") }}</el-tag
                  >
                </el-menu-item>
                <el-menu-item
                  index="/database"
                  :disabled="!isWorkflowRouteAllowed('/database')"
                ...
                >
                  ...
                  <el-tag
                    class="nav-state"
                    size="small"
                    :type="navigationTagType('/database', 'db_case')"
                    >{{ navigationStateLabel("/database", "db_case") }}</el-tag
                  >
                </el-menu-item>
                <el-menu-item
                  index="/functional"
                  :disabled="!isWorkflowRouteAllowed('/functional')"
                ...
                >
                  ...
                  <el-tag
                    class="nav-state"
                    size="small"
                    :type="navigationTagType('/functional', 'functional_case')"
                    >{{ navigationStateLabel("/functional", "functional_case") }}</el-tag
                  >
                </el-menu-item>
                <el-menu-item
                  index="/nfunctional"
                  :disabled="!isWorkflowRouteAllowed('/nfunctional')"
                ...
                >
                  ...
                  <el-tag
                    class="nav-state"
                    size="small"
                    :type="navigationTagType('/nfunctional', 'nonfunctional_case')"
                    >{{ navigationStateLabel("/nfunctional", "nonfunctional_case") }}</el-tag
                  >
                </el-menu-item>
                <el-menu-item
                  index="/acceptance"
                  :disabled="!isWorkflowRouteAllowed('/acceptance')"
                ...
                >
                  ...
                  <el-tag
                    class="nav-state"
                    size="small"
                    :type="navigationTagType('/acceptance', 'acceptance_case')"
                    >{{ navigationStateLabel("/acceptance", "acceptance_case") }}</el-tag
                  >
                </el-menu-item>
              </el-menu-item-group>
            </el-sub-menu>
            ...
            <el-menu-item-group title="平台说明">
              <el-menu-item
                index="/observability"
                :aria-current="route.path === '/observability' ? 'page' : undefined"
              >
...
// 工作区布局与项目导航，负责身份初始化、阶段提示和移动端键盘焦点管理。
...
import { ROUTE_PRESENTATION } from "@/shared/config/routePresentation";
...
type NavigationState =
  "loading" | "completed" | "current" | "locked" | "stale" | "available" | "regenerating";
...
/**
 * 派生用于界面展示或请求判断的当前路由 title。
 */
const currentRouteTitle = computed(() => ROUTE_PRESENTATION[route.path]?.title || "项目工作区");
/** 根据服务端允许路由判断是否有可进入的测试工作区。 */
const hasEnabledTestRoute = computed(() => testRoutes.some((path) => isWorkflowRouteAllowed(path)));
/** 仅在已加载状态且资料不处于 setup_required 时开放 Agent 导航。 */
const agentRouteReady = computed(
  () => workflowStatusLoaded.value && projectWorkflowStatus.value?.stage !== "setup_required",
);
/** 根据已加载阶段和计划重生成状态显示项目进度，访问许可仍由路由守卫与后端共同校验。 */
...
/** 单独判断项目分析；测试族任一相关步骤失效时提示整族导航需要更新。 */
const operationIsStale = (operation: string): boolean => {
  ...
  return staleOperations.value.some((item) => item === operation || item.startsWith(`${family}_`));
};

/** 按加载、重生成、失效、权限、当前页和完成状态的优先级派生导航标记。 */
...
/**
 * 移动抽屉打开时处理 Escape 关闭和 Tab 焦点循环，避免焦点跑到遮罩后的内容。
 * @param event 当前事件对象。
 */
const handleNavigationKeydown = (event: KeyboardEvent) => {
  ...
  const nextIndex = activeIndex < 0 ? 0 : (activeIndex + offset + targets.length) % targets.length;
  event.preventDefault();
  targets[nextIndex].focus();
};

/** 移动端选中导航后关闭抽屉并聚焦主内容区域。 */
...
/** 根据移动抽屉状态切换 body 标记，保持背景滚动与导航遮罩一致。 */
watch(
...
/** 要求当前项目身份并读取名称和工作流状态，未满足准备或导航条件时重定向。 */
...
/** 卸载时移除断点监听及抽屉 body 标记，不遗留全局界面副作用。 */
onBeforeUnmount(() => {
