...
TESTING_COMPONENTS = FRONTEND_ROOT / "src" / "components" / "testing"
TEST_PAGES = {
    ...
    "FounctionalTest.vue": ("functional_info", "functional_case", "session-only"),
...
def test_target_selector_keeps_long_selected_values_readable_without_focus_overlap():
    source = read(
        "ez_front_dev/src/components/testing/TestTargetSelector.vue"
    )
...
def test_all_eight_pages_share_anatomy_without_flattening_operations():
    for filename, contract in TEST_PAGES.items():
        source = read(f"ez_front_dev/src/components/{filename}")
...
def test_page_migration_removes_legacy_layout_and_readonly_textareas():
    ...
    execution = read("ez_front_dev/src/components/LlmWorkflowExecution.vue")
...
def test_workflow_controller_exposes_hydration_recovery_and_selection_stale_state():
    source = read("ez_front_dev/src/composables/useTestWorkflow.ts")
    for fragment in (
...
def test_results_are_visible_only_for_the_selection_that_generated_them():
    workflow = read("ez_front_dev/src/composables/useTestWorkflow.ts")
    for fragment in (
        "selectionMatches",
        "selectionsMatch",
    ...
    page_contracts = {
        ...
        "FounctionalTest.vue": ("currentFunctionalCaseSelection", "caseResultVisible", "caseExecutionVisible"),
    ...
    unit = read("ez_front_dev/src/components/UnitTest.vue")
    integration = read("ez_front_dev/src/components/IntegrationTest.vue")
    ...
    llm_workflow = read("ez_front_dev/src/composables/useLlmWorkflow.ts")
...
def test_unit_qualified_references_keep_raw_values_and_duplicate_labels_distinct():
    helper = read("ez_front_dev/src/ui/qualifiedReferences.ts")
    unit = read("ez_front_dev/src/components/UnitTest.vue")
...
def test_model_scope_and_session_persistence_contracts_remain_unchanged():
    ...
    for filename in ("UITest.vue", "DatabaseTest.vue", "AcceptanceTest.vue"):
        assert "persistResult: false" not in read(f"ez_front_dev/src/components/{filename}")
