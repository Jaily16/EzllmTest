...
from service.workflow.unit_reference import parse_unit_reference
...
async def _knowledge_bundle(
    ...
    *,
    ...
    """处理知识库数据包并返回现有契约规定的结果。

    参数:
        `context`：沿用签名中 `WorkflowContext` 类型约束的输入。
        `specs`：沿用签名中 `list[KnowledgeSpec]` 类型约束的输入。
...
async def _final_case_result(
    ...
    *,
    ...
    """处理最终用例结果并返回现有契约规定的结果。

    参数:
        `context`：沿用签名中 `WorkflowContext` 类型约束的输入。
        `prompt_text`：沿用签名中 `str` 类型约束的输入。
        `result`：沿用签名中 `dict[str, Any]` 类型约束的输入。
        `label`：沿用签名中 `str` 类型约束的输入。
...
async def _named_api_info(
    ...
    """处理命名API信息并返回现有契约规定的结果。

    参数:
        `context`：沿用签名中 `WorkflowContext` 类型约束的输入。
        `api_name`：沿用签名中 `str` 类型约束的输入。
...
async def _simple_case(
    ...
    *,
    ...
    """处理simple用例并返回现有契约规定的结果。

    参数:
        `context`：沿用签名中 `WorkflowContext` 类型约束的输入。
        `knowledge_spec`：沿用签名中 `KnowledgeSpec` 类型约束的输入。
        `info_key`：沿用签名中 `str` 类型约束的输入。
        `template`：沿用签名中 `Any` 类型约束的输入。
        `template_values`：沿用签名中 `dict[str, Any]` 类型约束的输入。
        `label`：沿用签名中 `str` 类型约束的输入。
