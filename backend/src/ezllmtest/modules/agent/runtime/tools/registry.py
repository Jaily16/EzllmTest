# 从不可变工作流目录生成统一工具定义，内部 Agent 与 MCP 使用相同的名称和输入约束。
...
from ezllmtest.modules.agent.domain.contracts import ToolRisk, TypedToolDefinition, typed_tool_definition_for
from ezllmtest.modules.agent.runtime.tools.schemas import EmptyToolInput, TOOL_INPUT_MODELS, ToolExecutionResult, WorkflowToolInput
from ezllmtest.modules.generation.public import profile_for
...
class RegisteredToolDefinition:
    ...
    def resolve_risks(self, *, regenerate: bool) -> frozenset[ToolRisk]:
        """强制重生成提升为付费、持久化和重生成风险集合，不能沿用较弱的普通调用授权。"""
        if regenerate:
            return frozenset(
                {ToolRisk.PAID, ToolRisk.PERSISTENT, ToolRisk.REGENERATE}
            )
...
class ToolRegistry:
    ...
    def workflow_definitions(self) -> tuple[RegisteredToolDefinition, ...]:
        """只返回具有工作流目录项的工具，三个只读状态工具不计入 19 workflows。"""
        return tuple(
...
def _read_tool(name: str, title: str, description: str):
    """构造无预算和持久化副作用的状态查询工具，风险固定为 READ_ONLY。"""
...
def _budget_for(operation: str) -> ToolBudgetMetadata:
    """从 map、structured 和 final 阶段预算档案构造工具公开预算信息。"""
...
def build_default_tool_registry() -> ToolRegistry:
    """组合三个只读状态工具与 19 个工作流工具；持久化能力决定风险和幂等语义。"""
    reads = (
        _read_tool(
...
def validate_workflow_input(
    ...
    """根据 operation 对应输入模型校验参数，调用工具前阻止未知字段和不合法选择。"""
