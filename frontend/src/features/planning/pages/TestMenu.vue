    ...
    <template v-else>
      ...
      >
        <div class="workspace-grid">
          <TestWorkspaceCard v-for="item in dashboardItems" :key="item.key" :item="item" />
        </div>
      </WorkspaceSection>
    </template>
  </div>
...
import TestWorkspaceCard from "@/features/planning/components/TestWorkspaceCard.vue";
import {
  TEST_WORKSPACES,
  type TestWorkspaceCardViewModel,
  type TestWorkspaceDefinition,
} from "@/entities/workflow/model/workspaces";
import {
  analysisMenu,
  analysisReady,
...
/**
 * 将工作流定义组合为卡片展示数据，保留禁用和失效提示。
 * @param definition 沿用当前 TypeScript 类型约束的输入。
...
const cardFor = (definition: TestWorkspaceDefinition): TestWorkspaceCardViewModel => {
  ...
  const hasSavedResult =
    projectWorkflowStatus.value?.completed_operations.includes(definition.terminalOperation) ===
    true;
...
/**
 * 派生用于界面展示或请求判断的测试菜单状态 announcement。
 */
