import { ElMessageBox } from "@/plugins/elementPlus";
import type { SetupGroup } from "@/state/projectSetup";
...
export const confirmSessionOnlyResultReset = async (
  label: string
): Promise<boolean> => {
  try {
    await ElMessageBox.confirm(
...
export const confirmPendingFileRemoval = async (
  filename: string
): Promise<boolean> => {
  try {
    await ElMessageBox.confirm(
