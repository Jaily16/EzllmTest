...
def test_trace_projection_is_truthful_and_has_optional_grafana_link():
    state = (FRONTEND / "src/state/agentWorkbench.ts").read_text(encoding="utf-8")
    workbench = (FRONTEND / "src/components/AgentWorkbench.vue").read_text(
        encoding="utf-8"
    )
...
def test_vite_defers_select_css_with_the_lazy_test_target_chunk():
    plugin = (FRONTEND / "src/plugins/elementPlus.ts").read_text(encoding="utf-8")
    selector = (FRONTEND / "src/components/testing/TestTargetSelector.vue").read_text(
        encoding="utf-8"
    )
    for stylesheet in (
