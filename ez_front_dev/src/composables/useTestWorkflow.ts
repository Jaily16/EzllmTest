...
export const normalizeSelections = (
  selection: Record<string, unknown>
): Record<string, unknown> => normalizeValue(selection) as Record<string, unknown>;
...
export const useTestWorkflow = ({ baseUrl, pid, steps: definitions }: UseTestWorkflowOptions) => {
  const steps = ref<TestWorkflowStep[]>(
    definitions.map((definition, index) => ({
      ...
      dependsOn:
        definition.dependsOn ?? (index === 0 ? [] : [definitions[index - 1].operation]),
  ...
  const refreshLockedStates = () => {
    steps.value.forEach((step) => {
      ...
      step.state = dependenciesReady(step) ? (step.state === "failed" ? "failed" : "ready") : "locked";
    });
  };
  ...
  const storedWorkflow = (): StoredWorkflow => ({
    ...
    steps: Object.fromEntries(
      steps.value.filter((step) => step.persistResult).map((step) => [
        step.operation,
        {
          state: step.state,
          result: step.result,
  ...
  const applyStatus = (status: ProjectWorkflowStatus, storedRevision: string | null) => {
    ...
    steps.value.forEach((step) => {
      if (status.stale_operations.includes(step.operation) || (sourceChanged && step.result !== null)) {
        step.state = "stale";
    ...
    while (changed) {
      ...
      steps.value.forEach((step) => {
        if (
          ...
          step.dependsOn.some((dependency) => dependency === operation || dependents.has(dependency))
        ) {
  ...
  const completeStep = (
    ...
    const selectionChanged =
      previousSelection !== undefined && JSON.stringify(previousSelection) !== JSON.stringify(normalized);
