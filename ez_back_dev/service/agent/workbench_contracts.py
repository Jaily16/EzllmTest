...
def _catalog_maximum(field: str) -> int:
    values: list[int] = []
    for definition in DEFAULT_TOOL_REGISTRY.workflow_definitions():
        if definition.budget is None:
            continue
        value = definition.budget.final.get(field)
        if isinstance(value, int) and value > 0:
            values.append(value)
    if not values:
        raise RuntimeError(f"workflow catalog has no positive {field} budget")
    return max(values)
...
def budget_for_preset(preset: AgentBudgetPreset | str) -> RunBudget:
    ...
    input_tokens = shape["max_model_calls"] * _catalog_maximum(
        "max_context_tokens"
    )
    output_tokens = shape["max_model_calls"] * _catalog_maximum(
        "final_output_tokens"
    )
    return RunBudget(
...
class AgentBudgetView(_PublicModel):
    ...
    def from_usage(
        ...
        limits = budget_for_preset(preset)
        return cls(
            preset=preset,
            limits=limits,
            usage=usage,
