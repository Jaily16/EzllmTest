"""19 个工作流的不可变语义目录，供分派、工具注册及契约核对共同使用。"""
...
class WorkflowDefinition:
    ...
    def __post_init__(self) -> None:
        """未显式指定提示版本时按 operation 生成稳定版本标识。"""
    ...
    def legacy_info_types(self) -> tuple[int, ...]:
        """将目录中的缓存产物映射回历史 info_type 编号，保持旧接口记录关联。"""
        return tuple(
...
def get_workflow_definition(operation: str) -> WorkflowDefinition:
    """从不可变操作索引获取工作流定义，未知 operation 直接失败。"""
...
def list_workflow_definitions() -> tuple[WorkflowDefinition, ...]:
    """返回完整不可变工作流元组，19 workflows 的顺序以此为准。"""
