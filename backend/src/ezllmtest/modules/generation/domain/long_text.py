# 按模型兼容能力和工作流语义选择长文本处理策略，区分完整语料分析与检索。
...
from ezllmtest.platform.ai.gateway import get_model_spec
from ezllmtest.modules.generation.domain.budget import BudgetStage, profile_for
...
class LongTextPolicy:
    ...
    def __post_init__(self) -> None:
        """校验长文本策略与操作类型一致，避免完整抽取误用只返回少量片段的检索策略。"""
...
def _artifact(
    ...
    """定义基于已有分析或知识生成最终产物的操作，不重复执行全项目资料抽取。"""
    return LongTextPolicy(
        operation=operation,
...
def get_long_text_policy(operation: str) -> LongTextPolicy:
    """按固定 operation 返回长文本处理政策，未知操作不能自行选择策略。"""
    try:
