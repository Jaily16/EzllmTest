...
TESTING_PAGES = FRONTEND_ROOT / "src" / "features" / "testing" / "pages"
...
def test_element_plus_is_explicitly_registered_without_full_runtime_or_css():
    ...
    plugin = read("ez_front_dev/src/shared/plugins/elementPlus.ts")
...
def test_runtime_element_plus_services_use_the_internal_plugin():
    runtime_sources = [
        FRONTEND_ROOT / "src" / "features" / "workspace" / "MainView.vue",
        FRONTEND_ROOT / "src" / "features" / "onboarding" / "views" / "CreateView.vue",
        FRONTEND_ROOT / "src" / "features" / "planning" / "views" / "TestPlan.vue",
        *sorted(TESTING_PAGES.glob("*Test.vue")),
        FRONTEND_ROOT / "src" / "shared" / "ui" / "confirmations.ts",
    ]
...
def test_accessibility_tokens_focus_motion_touch_and_import_order():
    ...
    accessibility = read("ez_front_dev/src/shared/styles/accessibility.css")

    imports = (
        "./shared/styles/tokens.css",
        "./shared/styles/element-plus-theme.css",
        "./shared/styles/base.css",
        "./shared/styles/accessibility.css",
    )
    positions = [main.index(item) for item in imports]
    assert positions == sorted(positions)
    assert "--ez-focus-color: #2f7d4a;" in tokens.lower()
...
def test_route_presentation_titles_and_focus_do_not_replace_guards():
    presentation = read("ez_front_dev/src/shared/config/routePresentation.ts")
    router = read("ez_front_dev/src/app/router/index.ts")
    main_view = read("ez_front_dev/src/features/workspace/MainView.vue")
    onboarding = read("ez_front_dev/src/features/onboarding/components/OnboardingShell.vue")
    about = read("ez_front_dev/src/features/about/AboutView.vue")
...
def test_form_controls_have_unique_ids_required_text_and_direct_names():
    model = read("ez_front_dev/src/shared/components/ModelSelector.vue")
    group = read("ez_front_dev/src/features/testing/components/TestFieldGroup.vue")
    target = read("ez_front_dev/src/features/testing/components/TestTargetSelector.vue")
...
def test_loading_skeletons_are_typed_decorative_and_only_used_for_read_recovery():
    feedback = read("ez_front_dev/src/shared/components/FeedbackState.vue")
    create = read("ez_front_dev/src/features/onboarding/views/CreateView.vue")
    plan = read("ez_front_dev/src/features/planning/views/TestPlan.vue")
    menu = read("ez_front_dev/src/features/planning/views/TestMenu.vue")
    scaffold = read("ez_front_dev/src/features/testing/components/TestWorkspaceScaffold.vue")
    login = read("ez_front_dev/src/features/onboarding/views/LoginView.vue")
...
def test_status_announcements_are_atomic_and_not_attached_to_whole_grids():
    stepper = read("ez_front_dev/src/features/workspace/components/WorkflowStepper.vue")
...
def test_user_facing_terminology_is_consistent_without_protocol_renames():
    ...
    workspace_sources = "\n".join(
        ...
        for path in sorted(TESTING_PAGES.glob("*Test.vue"))
    )
