# 作为可信宿主应用服务处理工作台命令和查询，不提供旧源码路径兼容转发。
...
from ezllmtest.modules.agent.domain.contracts import AgentRunStatus, ApprovalDecision, TrustedProjectScope
from ezllmtest.modules.agent.runtime.graph import graph_plan_hash
from ezllmtest.modules.agent.runtime.state.contracts import AGENT_GRAPH_VERSION
from ezllmtest.modules.agent.application.runtime import AgentRunHandle, AgentRuntimeService
from ezllmtest.modules.agent.runtime.tools.registry import DEFAULT_TOOL_REGISTRY
from ezllmtest.modules.agent.schemas.workbench import AgentApprovalView, AgentBudgetPreset, AgentBudgetView, AgentPlanStepView, AgentContextBindingView, AgentRunCreateRequest, AgentRunList, AgentRunSummary, AgentRunView, AgentTimelineEvent, budget_for_preset
from ezllmtest.modules.agent.ports.workbench import WorkbenchStore as AgentWorkbenchStore
...
class AgentWorkbenchService:
    def __init__(
        ...
        *,
        ...
        """绑定运行服务、工作台存储和可替换时钟，后续命令与事件使用同一状态所有者。"""
    ...
    def scope_hash(scope: TrustedProjectScope) -> str:
        """使用可信 scope 派生工作台存储边界，使不同项目或操作者的运行不可混用。"""
    ...
    async def _append(
        ...
        **data,
        ...
        """向当前 scope 的工作台时间线追加带时钟的展示事件，不把完整业务载荷作为状态广播。"""
    ...
    async def _clear_stale_active(
        ...
        """检查已有活动线程是否仍在运行，仅释放已失效的活动占位，不取消有效运行。"""
    ...
    async def create_run(
        ...
        """先预留项目活动槽，再创建运行并登记元数据；失败释放自己的预留，避免留下无法恢复的忙碌状态。"""
    ...
    async def get_run(
        ...
        """组合运行、预算和步骤证据形成脱敏工作台视图；展示不应直接暴露 checkpoint 或存储内部对象。"""
        scope_hash = self.scope_hash(scope)
        ...
        return AgentRunView(
            ...
            budget=AgentBudgetView.from_usage(metadata.budget_preset, usage, limits=run.budget),
    ...
    async def list_runs(
        ...
        *,
        ...
        """在 scope 内分页式组织已登记线程摘要，清理失效活动占位后给出当前活动线程。"""
        scope_hash = self.scope_hash(scope)
    ...
    async def decide_approval(
        ...
        """核验展示中的待审批运行，再把决定交给 runtime，并记录对应的工作台事件。"""
    ...
    async def edit_run(
        ...
        """只允许符合当前状态的运行编辑，提交给 runtime 后记录事件，不直接修改执行中的图对象。"""
        envelope = await self.runtime.get_run(scope, thread_id)
    ...
    async def cancel_run(
        ...
        """区分待审批与执行中状态选择取消路径，记录操作者意图而不伪造工具已停止或结果已保存。"""
        view = await self.get_run(scope, thread_id)
        if view is None:
            raise KeyError("Agent thread was not found")
    ...
    async def recover_run(
        ...
        """恢复前重新预留活动槽并检查运行冲突，成功绑定线程，失败释放本次预留。"""
        scope_hash = self.scope_hash(scope)
        active = await self._clear_stale_active(scope, scope_hash)
    ...
    async def replay_events(
        ...
        """读取 scope 内的工作台时间线供 SSE 恢复，不以重放请求触发新的 Agent 执行。"""
