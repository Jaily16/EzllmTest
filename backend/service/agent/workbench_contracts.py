...
from infrastructure.llm.gateway import list_model_labels
from infrastructure.runtime_config import BUDGET_DEFAULTS, positive_integer
...
from service.agent.tool_schemas import ToolRetrievalEvidence
from infrastructure.observability.agent_telemetry import telemetry_public_status
...
def budget_for_preset(preset: AgentBudgetPreset | str) -> RunBudget:
    ...
    参数:
        `preset`：沿用签名中 `AgentBudgetPreset | str` 类型约束的输入。

    返回:
        `RunBudget`，内容保持现有调用方契约。"""
    ...
    input_tokens = positive_integer(input_name, os.environ.get(input_name, str(BUDGET_DEFAULTS[input_name])))
    output_tokens = positive_integer(output_name, os.environ.get(output_name, str(BUDGET_DEFAULTS[output_name])))
    return RunBudget(
...
class AgentBudgetView(_PublicModel):
    ...
    def from_usage(
        ...
        *,
        ...
        参数:
            `preset`：沿用签名中 `AgentBudgetPreset` 类型约束的输入。
            `usage`：沿用签名中 `UsageCounters` 类型约束的输入。
            `limits`：沿用签名中 `RunBudget` 类型约束的输入。

        返回:
            `'AgentBudgetView'`，内容保持现有调用方契约。"""
        return cls(
            preset=preset,
            limits=limits,
            usage=usage,
...
class AgentStepEvidence(_PublicModel):
    ...
    def _retention_matches_result(self):
        """处理retention匹配关系结果，并保持 `AgentStepEvidence` 的现有状态约束。
...
def public_capabilities() -> dict[str, JsonValue]:
    """处理公开能力清单并返回现有契约规定的结果。

    返回:
        `dict[str, JsonValue]`，内容保持现有调用方契约。"""
