# 为 Agent 组织有预算的项目检索，并收集不含全文的引用证据。
...
from ezllmtest.modules.knowledge.schemas.contracts import RetrievalCitation, RetrievalQueryEvidence, TrustedProjectScopeLike, ToolRetrievalEvidence
...
def _normalized_text(value: str) -> str:
    """以 Unicode NFKC 和空白折叠规范文档文本，供检索去重和身份计算。"""
    return " ".join(unicodedata.normalize("NFKC", value).split())


def bm25_tokens(value: str) -> tuple[str, ...]:
    """把查询与文档规范化为检索词项，离线评分不调用模型。"""
...
def _bm25_scores(query: str, documents: Sequence[Document]) -> dict[str, float]:
    """按词项频率和文档长度计算 BM25 分数，供已有候选的相关性排序。"""
...
def _safe_source_label(metadata: dict[str, Any]) -> str:
    """来源只保留文件名，剔除控制字符并限制长度，不把本机完整路径写入证据。"""
...
def _scope_hash(scope: TrustedProjectScopeLike) -> str:
    """把项目、操作者和 scope 版本共同纳入检索会话身份，避免跨授权范围混用。"""
    value = "\x00".join(
...
def active_agent_retrieval(project_id: str) -> AgentRetrievalSession | None:
    """读取活动检索会话时再次核对项目，防止把其他项目检索归入当前证据。"""
...
# 检索结果携带 source revision、策略和 citation metadata，避免 stale 内容伪装成当前证据。
class AgentEvidenceRetriever:
    def __init__(
        ...
        *,
        ...
        """固定本次检索的文档集、语料类别及来源 revision，并复用会话和 token 计数器跟踪上下文预算。"""
        self._index = index
        self._policy = policy
    ...
    def _ranked(self, query: str) -> list[tuple[Document, float, str]]:
        """在当前项目候选中结合评分和限额选择证据，保留来源身份用于引用。"""
        ...
        if self._session.strategy == "hybrid_rerank_v1":
            ...
            def rerank_key(key: str):
                """重排时优先考虑标识符匹配和查询词覆盖，再使用融合得分及稳定 ID 排序。"""
    ...
    def _invoke(self, query: str, config=None, **_kwargs) -> list[Document]:
        """执行受 session 约束的检索并收集允许的引用元数据，不向观测复制检索正文。"""
        del config
    ...
    def invoke(self, query: str, config=None, **kwargs) -> list[Document]:
        """在调用边界核验可信项目并执行证据检索，复用同一会话策略与预算约束。"""
