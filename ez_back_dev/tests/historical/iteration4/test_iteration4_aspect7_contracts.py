...
FIXTURES = fixture_path("historical", "iteration4")
...
def test_aspect7_manifest_chains_history_and_freezes_current_delivery_files():
    ...
    assert manifest["delivery"]["compose_sha256"].isalnum()
...
def test_aspect7_documents_state_security_operations_and_stop_boundary():
    contract = canonical_document_path(
        "docs/iteration-4-aspect-7-observability-delivery-contract.md"
    ).read_text(
        encoding="utf-8"
    )
    compose = canonical_document_path("docs/iteration-4-compose.md").read_text(encoding="utf-8")
    for required in (
