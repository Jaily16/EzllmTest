...
def build_stream_request(
    ...
    *,
    ...
    参数:
        `name`：目标名称。
        `prompt`：沿用签名中 `str` 类型约束的输入。
        `max_tokens`：沿用签名中 `int` 类型约束的输入。
        `request_options`：沿用签名中 `dict[str, Any] | None` 类型约束的输入。
...
async def _stream_chat_completion_impl(
    ...
    *,
    ...
    """流式处理对话completion IMPL，并遵循现有调用契约。
    ...
    返回:
        `AsyncIterator[ModelStreamEvent]`，内容保持现有调用方契约。

    异常:
        `LLMTruncatedResponseError, LLMProviderError, LLMEmptyResponseError, LLMTimeoutError, LLMRateLimitError, provider_request_error`：输入、状态或下游结果不满足现有约束时抛出。
...
def ensure_non_empty_response(name: str, content: str) -> str:
    ...
    异常:
        `LLMEmptyResponseError`：输入、状态或下游结果不满足现有约束时抛出。"""
