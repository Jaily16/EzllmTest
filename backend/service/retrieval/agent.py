...
class AgentEvidenceRetriever:
    def __init__(
        ...
        *,
        ...
        参数:
            `index`：沿用签名中 `Any` 类型约束的输入。
            `policy`：沿用签名中 `Any` 类型约束的输入。
            `documents`：沿用签名中 `Sequence[Document]` 类型约束的输入。
            `corpus`：沿用签名中 `Literal['design', 'requirements', 'knowledge']` 类型约束的输入。
            `source_revision`：沿用签名中 `str` 类型约束的输入。
            `index_status`：沿用签名中 `Literal['build', 'reuse']` 类型约束的输入。
            `session`：沿用签名中 `AgentRetrievalSession` 类型约束的输入。
            `token_counter`：调用方传入的现有参数。"""
        self._index = index
        self._policy = policy
    ...
    def _ranked(self, query: str) -> list[tuple[Document, float, str]]:
        ...
        返回:
            `list[tuple[Document, float, str]]`，内容保持现有调用方契约。"""
