...
def test_compose_pins_external_images_and_only_publishes_loopback_ports():
    ...
    for name in ("mysql", "redis", "otel-collector", "prometheus", "tempo", "grafana"):
        ...
        assert image == contract["images"]["refs"][name]
    assert services["legacy-api"]["image"] == contract["images"]["application_tags"]["backend"]
    assert services["agent-api"]["image"] == contract["images"]["application_tags"]["backend"]
    assert services["worker"]["image"] == contract["images"]["application_tags"]["backend"]
    assert services["frontend"]["image"] == contract["images"]["application_tags"]["frontend"]
...
def test_docker_context_excludes_credentials_dependencies_and_project_files():
    ...
    assert contract["dockerfiles"]["backend_base"] in backend
