...
import type { SetupGroup, SetupStepState } from "@/entities/project/model/setup";
import {
...
/** 识别 ASCII 控制字符，供项目名称校验阻止不可见路径输入。 */
...
/** 仅 setup_complete 阶段允许界面继续计划流程，不能因本地有项目 ID 就视为资料齐全。 */
...
/**
 * 提取 API 允许展示的错误原因，普通异常或未知错误使用对应回退文案。
...
/**
 * 校验扩展名、组内大小上限与规范化重名，失败同时标记文件和组级错误。
...
/**
 * 请求创建项目并立即保存返回 ID，使后续上传失败仍可继续原项目。
 * @returns 保持当前 TypeScript 返回类型与调用方约定。
 */
...
/**
 * 按组逐文件上传，成功立即登记；中途失败保留已成功项供下次跳过。
 * @param group 沿用当前 TypeScript 类型约束的输入。
...
/**
 * 防重复提交，确认后依次创建、上传三组并确认准备状态；该流程不启动模型分析。
 * @returns 保持当前 TypeScript 返回类型与调用方约定。
 */
...
/** 请求返回首页，离开保护由页面导航守卫处理。 */
const requestExit = async (): Promise<void> => {
  await router.push("/");
};

/** 资料请求进行中触发浏览器离开提示，避免用户误认为上传已完成。 */
...
/** 上传或确认进行中阻止离开；已有未完成项目时要求确认退出准备流程。 */
...
/** 挂载时安装离开提示并核对已保存项目的准备状态，不自动重新上传资料。 */
onMounted(async () => {
