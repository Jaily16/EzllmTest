# 提供流式阶段共用的上下文、事件与错误处理，避免不同工作流偏离保存边界。
...
from ezllmtest.modules.generation.domain.prompts.templates import build_grounded_prompt, build_structured_output_prompt
from ezllmtest.platform.ai.stream import FINAL_MAX_TOKENS, INTERMEDIATE_MAX_TOKENS, TokenUsage, combine_token_usage, ensure_non_empty_response, stream_chat_completion
from ezllmtest.platform.ai.gateway import provider_options
from ezllmtest.modules.generation.application.test_plan import TEST_PLAN_MINIMUM_TIMEOUT_SECONDS
from ezllmtest.modules.generation.domain.budget import bound_prompt_context, profile_for, stage_for_model_call
from ezllmtest.modules.generation.domain.long_text import effective_context_budget
...
def event(event_name: str, **data: Any) -> dict[str, Any]:
    """构造统一 SSE 事件信封；此处只组织事件类型和数据，不进行网络写入。"""
...
async def ensure_connected(context: WorkflowContext) -> None:
    """在付费或保存边界前检查客户端是否断开，断开后通过取消路径停止后续工作。"""
...
def require_source_revision(context: WorkflowContext) -> str:
    """要求工作流上下文已有来源 revision，防止无版本身份的检索或缓存复用。"""
...
def parse_structured_result(
    ...
    """清理模型可能附带的代码围栏后按 Pydantic schema 验证 JSON，失败作为结构化结果错误而不保存草稿。"""
...
def structured_prompt(prompt_text: str, model: type[BaseModel]) -> str:
    """用目标模型的 JSON schema 生成结构化输出要求；schema 描述属于实际提示词输入，不能作为普通注释改写。"""
...
async def stream_model_call(
    ...
    *,
    ...
    """为当前阶段选择预算和 provider 参数，先约束上下文再流式调用；聚合完整响应与用量，空结果不能进入成功保存路径。"""
...
def _split_text_within_budget(
    ...
    """按指定 tokenizer 预算切分超长文本，优先利用换行边界，保证每段能够进入后续上下文。"""
    if not text:
        return []
...
def partition_texts_within_budget(
    ...
    *,
    ...
    """把文本按 token 上限装入批次，超大单段先拆分，避免 map/reduce 请求超预算。"""
...
def split_documents_within_budget(
    ...
    *,
    ...
    """切分文档正文时复制来源元数据和片段位置，使后续证据仍能追溯原文。"""
...
async def stream_map_reduce(
    ...
    *,
    ...
    """在阶段预算内逐批提取并归并文档结果，批间检查断连；归并无法收敛时明确失败而不无限调用。"""
    if not documents:
        raise WorkflowStreamError(
            "documents_empty",
            "文档拆分或检索后没有可分析的内容",
...
def rag_prompt(question: str, context_text: str) -> str:
    """将检索上下文交给统一 grounded prompt 构造器，保持证据约束与引用格式。"""
