<template>
  <el-row>
    <el-button style="width: 180px;" type="primary" :icon="Right" @click="get_ui_info" plain
      round>开始前端UI测试分析</el-button>
  </el-row>
  <el-row v-loading="loading1" element-loading-text="LLM正在进行UI接口测试相关分析,请耐心等待"
    element-loading-background="rgba(255,255,255,1)" style="margin-top: 10px; width: 99%; z-index: 900" />
  <el-row v-if="show1">
    <span class="cn_name">LLM从知识库中找到有关系统中前端UI设计的的相关内容</span>
  </el-row>
  <el-row v-if="show1" style="margin-top: 10px;">
    <el-input v-model="ui_info" style="width: 99%; margin-top: 10px;" :autosize="{ minRows: 2, maxRows: 20 }"
      type="textarea" disabled />
  </el-row>
  <el-row v-if="show1">
    <el-button style="margin-top: 20px; width: 180px;" type="success" :icon="Right" @click="startUITest" plain
      round>生成前端UI测试用例</el-button>
  </el-row>
  <el-divider v-if="show1" />
  <el-row v-loading="loading2" element-loading-text="LLM正在综合知识库内容进行UI测试分析"
    element-loading-background="rgba(255,255,255,1)" style="margin-top: 10px; width: 99%; z-index: 902" />
  <el-row v-if="show2">
    <span class="cn_name">LLM结合知识库查找有关前端UI测试的内容...</span>
  </el-row>
...
const get_ui_info = () => {
  show1.value = false
  loading1.value = true
  axios.get(requestUrl + "/project/llm/ui/info/" + project_id).then((resp) => {
    if (resp.data.data == false) {
      ElMessage({ message: resp.data.reason, type: "error" });
      return
    }
    else {
      ui_info.value = resp.data.data
...
const startUITest = () => {
  ...
  knowledge.value = ''
  cases.value = ''
  axios.post(requestUrl + "/project/llm/ui/case", {
    pid: project_id,
    info: ui_info.value,
  }).then((resp) => {
    if(resp.data.data == false){
      ...
      knowledge.value = resp.data.data.ui_test_knowledge
