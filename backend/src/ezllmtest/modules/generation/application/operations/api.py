# 实现API 接口相关生成操作，复用统一模型和知识接口；有效结果保存遵循上层工作流边界。
...
from ezllmtest.modules.generation.schemas.analysis import ApiList
from ezllmtest.modules.generation.domain.prompts.templates import API_TEST_INFO_TEMPLATE, API_TEST_GENERATE_TEST_CASE_TEMPLATE, APIS_TEST_GENERATE_TEST_CASE_TEMPLATE, API_TEST_INFO_MAP_REDUCE_TEMPLATE
import ezllmtest.modules.projects.public as documentTools
from ezllmtest.modules.projects.public import InfoType
from ezllmtest.modules.knowledge.public import api_retriever
...
def find_out_apis_info(pid: str):
    """优先复用 API 设计摘要，缺失时完整分析设计文档，再生成结构化接口列表。"""
    try:
...
def find_out_api_info(pid: str, api_name: str):
    """按 API 名称检索当前项目设计文档，基于命中上下文补充该接口分析。"""
    try:
...
def find_api_test_knowledge(pid: str):
    """复用已登记 API 测试知识，未命中时从项目知识库检索后登记。"""
    try:
...
def generate_api_test_cases(pid: str, info: str, test_type: int, output_type: int, api_name: str = ''):
    """指定 API 时重新检索该接口信息，再组合测试知识、范围和输出格式生成用例。"""
    try:
        if api_name != '':
