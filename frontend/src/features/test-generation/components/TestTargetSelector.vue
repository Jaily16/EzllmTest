<template>
  ...
  >
    ...
    <div v-if="selectedOption" :id="selectionDescriptionId" class="test-target-selector__summary">
...
const value = computed({
  /** 从父组件读取当前选择值，不维护第二份目标状态。 */
  get: () => props.modelValue,
  /** 通过 update:modelValue 通知父组件目标变化。 */
...
/** 下拉选择器可见性变化时通知父层，以便配合交互状态。 */
const handleVisibleChange = (visible: boolean) => {
  dropdownOpen.value = visible;
};
/**
 * 派生用于界面展示或请求判断的选择条件描述 ID。
 */
const selectionDescriptionId = computed(() => `${props.fieldId}-selection-description`);
/**
 * 派生用于界面展示或请求判断的described by。
 */
...
.test-target-selector__control--selected:not(.test-target-selector__control--open)
