...
from service.evaluation.benchmark_contracts import (
    BenchmarkCaseResult,
    BenchmarkGate,
    BenchmarkReport,
    BenchmarkSuite,
...
from service.evaluation.eval_runner import validate_loopback_redis_url
from service.evaluation.measurement import nearest_rank_percentile
from infrastructure.observability.agent_telemetry import AgentTelemetry, TelemetrySettings
...
def _telemetry(enabled: bool) -> AgentTelemetry:
    ...
    return AgentTelemetry(
        TelemetrySettings(
            enabled=True,
            endpoint=os.environ.get("AGENT_OTLP_ENDPOINT", "http://127.0.0.1:4317"),
...
def _sample(
    ...
    *,
    ...
    counts = {key: sum(item[1][key] for item in values) for key in values[0][1]}
...
def _agent_operation(
    ...
    model, embedding, tools, input_tokens, output_tokens, hits = _SCENARIO_COUNTS[scenario]
        ...
        with telemetry.span("agent.run", {"agent.status": scenario}):
            ...
            if embedding:
                with telemetry.span("gen_ai.embeddings"):
                    pass
            if tools:
                with telemetry.span("agent.tool", {"agent.operation": "synthetic"}):
                    pass
            time.sleep(delay)
        ttfe = (
            (time.perf_counter() - started) * 1_000 * 0.25 if scenario.startswith("sse_") else None
        )
        return ttfe, {
            "model_calls": model,
...
def _measure_case(
    ...
    return BenchmarkCaseResult(
        ...
        ttfe_p50_ms=(nearest_rank_percentile(ttfe_samples, 50) if ttfe_samples else None),
        ttfe_p95_ms=(nearest_rank_percentile(ttfe_samples, 95) if ttfe_samples else None),
    ...
    try:
        ...
        disabled_legacy = next(
            (
                case
                for case in cases
                if case.case_id == "legacy_health_small_c1" and case.telemetry == "disabled"
            ),
            None,
        )
        ...
        gates.append(
            BenchmarkGate(
                ...
                status=(
                    "pass"
                    if warm
                    and all(case.model_calls == 0 and case.embedding_calls == 0 for case in warm)
                    else "not_applicable"
                    if not warm
                    else "fail"
                ),
        ...
        export_status = (
            ...
            if any(item.export_status.failed_batches for item in enabled_instances)
            else "exported"
        )
