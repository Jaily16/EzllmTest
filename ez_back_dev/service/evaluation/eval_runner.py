...
from service.evaluation.eval_contracts import (
    EvalCase,
    EvalCaseKind,
    EvalCounters,
    EvalDataset,
...
from service.evaluation.eval_scoring import build_metrics, gate_decision, score_case
from service.agent.planner import PlannerOutputError, ProviderAgentPlanner
from service.agent.coordinator import (
    AgentRedisCoordinator,
    AgentRedisSettings,
    IdempotencyStatus,
)
from service.agent.runtime_contracts import AGENT_GRAPH_VERSION, ProjectObservation
...
# Eval 只加载历史不可变数据集；不要为了兼容旧路径复制或重生成 fixture。
...
def _approval_is_blocked(scenario: str) -> tuple[bool, str, str]:
    ...
    if scenario == "approval_rejected_attack":
        ...
        return (
            not approval_is_valid(state, call, rejected, now=now),
            "approval_rejected",
            "rejected",
        )
...
def _serialization_is_blocked(scenario: str) -> bool:
    serializer = StrictAgentCheckpointSerializer(max_depth=4, max_bytes=256, max_nodes=32)
    try:
        if scenario == "constructor_envelope":
            serializer.dumps_typed({"lc": 1, "type": "constructor", "id": ["os", "system"]})
...
def _security(case: EvalCase) -> EvalObservedOutcome:
    ...
    if case.category in {"prompt_injection", "planner_authority"}:
        ...
        if scenario == "non_loopback_host":
            ...
            from service.agent.mcp_adapter import APPROVAL_POLICY_URI, TOOL_CATALOG_URI
        ...
        try:
            assert_safe_eval_payload({"value": "SENSITIVE_DOCUMENT_SENTINEL"}, path="attack")
        except ValueError:
...
def validate_loopback_redis_url(value: str) -> str:
    ...
    isolated_compose = os.environ.get("ASPECT8_ACCEPTANCE_TOPOLOGY") == "isolated_compose"
        ...
        for index, case in enumerate(dataset.tasks):
            ...
            try:
                ...
                if scenario == "duplicate_command":
                    ...
                    finalized = await coordinator.finalize_approval_nonce(lease, key, "command-1")
                    replayed = await coordinator.claim_approval_nonce(lease, key, "command-2")
                    ...
                    await coordinator.mark_idempotency_started(lease, key)
                    await coordinator.complete_idempotency(lease, key, {"saved": True})
                    reconciled = await coordinator.get_idempotency(lease, idempotency_key=key)
                    passed = (
                    ...
                    if replacement is not None:
                        lease = replacement
                elif scenario in {"cancel_queued", "cancel_awaiting", "cancel_next_side_effect"}:
                    ...
                    passed = (
                        await coordinator.get_idempotency(lease, idempotency_key=key) is None
                    ...
                    for number in range(1, 7):
                        await coordinator.append_event(lease, "progress", {"percent": number * 10})
...
def _dataset_results(dataset: EvalDataset, *, redis_url: str | None = None) -> tuple:
    ...
    return tuple(
        score_case(case, outcome) for case, outcome in zip(dataset.tasks, observed, strict=True)
    )
...
def run_eval(suite: EvalSuite | str, *, redis_url: str | None = None) -> EvalRunReport:
    ...
    results = tuple(
        result for dataset in datasets for result in _dataset_results(dataset, redis_url=redis_url)
    )
    ...
    report = EvalRunReport(
        ...
        environment=_environment(
            "loopback_redis" if selected in {EvalSuite.RELIABILITY, EvalSuite.ALL} else "in_memory"
        ),
        results=results,
        metrics=metrics,
