...
from infrastructure.persistence.artifact_repository import Session
...
from service.project.setup import ProjectSetupPersistenceError
...
def derive_project_stage(
    ...
    *,
    ...
    参数:
        `setup`：沿用签名中 `bool` 类型约束的输入。
        `analysis`：沿用签名中 `bool` 类型约束的输入。
        `artifact_count`：沿用签名中 `int` 类型约束的输入。
        `completed_operations`：沿用签名中 `Collection[str]` 类型约束的输入。
        `required_operations`：沿用签名中 `Collection[str]` 类型约束的输入。

    返回:
        `ProjectStage`，内容保持现有调用方契约。"""
...
def _parse_menu(value: Any) -> dict[str, bool] | None:
    ...
    返回:
        `dict[str, bool] | None`，内容保持现有调用方契约。"""
...
def _load_artifact_facts(
    ...
    参数:
        `pid`：项目 ID。
        `current_source_revision`：沿用签名中 `str | None` 类型约束的输入。

    返回:
        `tuple[ArtifactFact, ...]`，内容保持现有调用方契约。

    异常:
        `ProjectSetupPersistenceError`：输入、状态或下游结果不满足现有约束时抛出。"""
    session = Session()
    try:
        rows = (
...
def _legacy_analysis_bundle(pid: str) -> tuple[bool, dict[str, bool] | None]:
    """处理legacy分析数据包并返回现有契约规定的结果。

    参数:
        `pid`：项目 ID。

    返回:
        `tuple[bool, dict[str, bool] | None]`，内容保持现有调用方契约。"""
...
def get_project_workflow_status(pid: str) -> ProjectWorkflowStatus:
    ...
    返回:
        `ProjectWorkflowStatus`，内容保持现有调用方契约。"""
