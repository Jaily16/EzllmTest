...
def generate_test_cases(unit_test_knowledge: str, static_method: str, unit_test_method_knowledge: str,
    ...
    参数:
        `unit_test_knowledge`：沿用签名中 `str` 类型约束的输入。
        `static_method`：沿用签名中 `str` 类型约束的输入。
        `unit_test_method_knowledge`：沿用签名中 `str` 类型约束的输入。
        `unit`：沿用签名中 `str` 类型约束的输入。
        `unit_info`：沿用签名中 `str` 类型约束的输入。
