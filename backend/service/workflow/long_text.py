...
from infrastructure.llm.gateway import get_model_spec
from service.workflow.budget import BudgetStage, profile_for
...
def get_long_text_policy(operation: str) -> LongTextPolicy:
    """获取LONG文本策略，并遵循现有调用契约。

    参数:
        `operation`：工作流操作名。

    返回:
        `LongTextPolicy`，内容保持现有调用方契约。
...
def strategy_for(
    ...
    *,
    ...
    参数:
        `operation`：工作流操作名。
        `source_tokens`：沿用签名中 `int` 类型约束的输入。
        `source_mode`：沿用签名中 `SourceMode | None` 类型约束的输入。

    返回:
        `LongTextStrategy`，内容保持现有调用方契约。
