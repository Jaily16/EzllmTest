# 从文档结构提取确定性测试证据，供测试菜单约束判断，不运行模型。
...
from ezllmtest.modules.generation.public import ProjectTestEvidence, TestMenu
...
def _document_texts(documents: Iterable[Any]) -> list[str]:
    """从当前项目已选资料提取确定性结构证据，不通过模型猜测测试菜单支持范围。"""
    values: list[str] = []
...
def collect_project_test_evidence(
    ...
    """对已提供文档使用固定模式提取单元、集成和关系信号，不调用模型。"""
...
def reconcile_test_menu(
    ...
    """达到单元或集成结构证据阈值时补开菜单项，不因候选模型遗漏而丢失可验证测试类型。"""
