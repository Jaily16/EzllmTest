...
// 计划和测试菜单页面消费项目共享状态；生成结果完整保存后才开放相应测试流程。
...
import { DEFAULT_MODEL, MODEL_OPTIONS, type ModelLabel } from "@/shared/config/models";
import { TEST_WORKSPACES } from "@/entities/workflow/model/workspaces";
import {
...
import { confirmProjectAnalysisRegeneration } from "@/features/planning/ui/confirmations";
...
/** 中止旧读取后加载已保存三件套，只有当前请求能够结束本轮加载状态。 */
const loadSavedBundle = async () => {
  ...
  try {
    ...
    bundleError.value =
      caught instanceof Error ? caught.message : "已保存计划读取失败，请稍后重试。";
  } finally {
...
/** 按服务端完成或失效记录决定是否恢复旧计划，不因进入页面自动运行模型。 */
const hydratePlan = async () => {
  ...
  try {
    ...
    bundleError.value =
      caught instanceof Error ? caught.message : "项目分析状态读取失败，请稍后重试。";
    bundleLoading.value = false;
  }
};

/** 替换计划前取得用户确认，再开启重生成请求。 */
...
/**
 * 保存旧三件套；仅在完整、就绪且后端确认保存后替换，失败或取消保留旧版本与本次草稿。
