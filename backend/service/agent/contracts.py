...
class ApprovalBinding(_ContractModel):
    ...
    def _expiry_follows_decision(self):
        """处理过期时间顺序关系决策，并保持 `ApprovalBinding` 的现有状态约束。
...
class AgentRunState(_ContractModel):
    ...
    def _plan_fits_budget(self):
        """处理计划适配关系预算，并保持 `AgentRunState` 的现有状态约束。
...
def approval_binding_for(
    ...
    *,
    ...
    参数:
        ...
        `call`：沿用签名中 `PlannedToolCall` 类型约束的输入。
        `decision`：沿用签名中 `ApprovalDecision` 类型约束的输入。
        `decided_at`：沿用签名中 `datetime` 类型约束的输入。
        `expires_at`：沿用签名中 `datetime` 类型约束的输入。
        `nonce`：沿用签名中 `str` 类型约束的输入。

    返回:
        `ApprovalBinding`，内容保持现有调用方契约。
...
def approval_is_valid(
    ...
    *,
    ...
    """校验审批是否仍绑定当前参数、风险、预算、revision 与有效期。

    参数:
        `state`：沿用签名中 `AgentRunState` 类型约束的输入。
        `call`：沿用签名中 `PlannedToolCall` 类型约束的输入。
        `approval`：沿用签名中 `ApprovalBinding` 类型约束的输入。
        `now`：沿用签名中 `datetime | None` 类型约束的输入。
...
def _validated_usage(
    ...
    参数:
        `previous`：沿用签名中 `UsageCounters` 类型约束的输入。
        `current`：沿用签名中 `UsageCounters | None` 类型约束的输入。
...
def transition(
    ...
    参数:
        `state`：沿用签名中 `AgentRunState` 类型约束的输入。
        `event`：沿用签名中 `AgentTransitionEvent` 类型约束的输入。

    返回:
        `AgentRunState`，内容保持现有调用方契约。
