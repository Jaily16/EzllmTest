...
from service.agent.context import AgentContextAssembler, ContextAssemblyError
from service.agent.budget import reserve_tool_budget
from service.agent.planner import AgentPlanner, compile_planned_calls
...
from service.agent.tool_schemas import ToolExecutionResult, ToolExecutionStatus
...
def _usage_after_tool(
    ...
    参数:
        `envelope`：沿用签名中 `AgentGraphEnvelope` 类型约束的输入。
        `result`：沿用签名中 `ToolExecutionResult` 类型约束的输入。
...
def build_agent_graph(
    *,
    ...
    """构建保持审批、预算、恢复与工具边界的 Agent 状态图。

    参数:
        `observer`：沿用签名中 `Callable[[Any], Any]` 类型约束的输入。
        `planner`：沿用签名中 `AgentPlanner` 类型约束的输入。
        `tool_adapter`：沿用签名中 `Any` 类型约束的输入。
        `checkpointer`：沿用签名中 `Any` 类型约束的输入。
        `clock`：沿用签名中 `Callable[[], datetime]` 类型约束的输入。
        `cancel_checker`：沿用签名中 `Callable[[AgentRunState], Any] | None` 类型约束的输入。
        `progress_sink`：沿用签名中 `Callable[[AgentRunState, Any], Any] | None` 类型约束的输入。
        `approval_ttl`：沿用签名中 `timedelta` 类型约束的输入。
    ...
    async def observe(state: AgentGraphState) -> AgentGraphState:
        """记录 Agent 节点执行结果的安全观测数据。

        参数:
            `state`：沿用签名中 `AgentGraphState` 类型约束的输入。

        返回:
            `AgentGraphState`，内容保持现有调用方契约。"""
