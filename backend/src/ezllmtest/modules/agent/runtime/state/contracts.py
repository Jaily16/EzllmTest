# 限制运行状态为可严格校验的 JSON，并定义取消、失败及 outcome_unknown 的恢复语义。
...
from ezllmtest.modules.agent.domain.contracts import AgentRunState, ApprovalBinding, ApprovalDecision, ToolRisk
...
class ApprovalResume(_RuntimeModel):
    ...
    def _decision_payload_is_coherent(self):
        """核验恢复决定与编辑/审批载荷是否匹配，不能用相互矛盾的字段推进状态。"""
...
class AgentGraphEnvelope(_RuntimeModel):
    ...
    def _temporal_and_revision_binding(self):
        """校验图状态的时间顺序与来源 revision 绑定，过时上下文不得被当作当前执行状态。"""
