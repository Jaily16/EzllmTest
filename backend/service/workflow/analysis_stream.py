...
async def _map_reduce_value(
    ...
    *,
    ...
    参数:
        `context`：沿用签名中 `WorkflowContext` 类型约束的输入。
        `documents`：沿用签名中 `list[Any]` 类型约束的输入。
...
async def stream_configured_analysis(
    ...
    *,
    ...
    """流式处理已配置的分析，并遵循现有调用契约。

    参数:
        `context`：沿用签名中 `WorkflowContext` 类型约束的输入。
        `spec`：沿用签名中 `AnalysisSpec` 类型约束的输入。
        `include_structure`：沿用签名中 `bool` 类型约束的输入。
