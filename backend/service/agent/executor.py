...
def _safe_error(
    ...
    *,
    ...
    参数:
        ...
        `operation`：工作流操作名。
        `code`：沿用签名中 `str` 类型约束的输入。
        `category`：沿用签名中 `str` 类型约束的输入。
        `message`：沿用签名中 `str` 类型约束的输入。
        `retryable`：沿用签名中 `bool` 类型约束的输入。
        `status`：沿用签名中 `ToolExecutionStatus` 类型约束的输入。
        `invoked`：沿用签名中 `bool` 类型约束的输入。
...
class AgentToolExecutor:
    ...
    async def execute(
        ...
        参数:
            ...
            `context`：沿用签名中 `ToolInvocationContext` 类型约束的输入。
    ...
    def _approval_error(
        ...
        """处理审批错误，并保持 `AgentToolExecutor` 的现有状态约束。
        ...
        返回:
            `ToolExecutionResult | None`，内容保持现有调用方契约。"""
    ...
    async def _execute_workflow(
        ...
        async def disconnected() -> bool:
            """处理断开状态，并保持 `AgentToolExecutor` 的现有状态约束。"""
    ...
    async def _consume_stream(
        ...
        参数:
            `definition`：沿用签名中 `RegisteredToolDefinition` 类型约束的输入。
            `stream`：沿用签名中 `AsyncIterator[dict[str, Any]]` 类型约束的输入。
            `context`：沿用签名中 `ToolInvocationContext` 类型约束的输入。
