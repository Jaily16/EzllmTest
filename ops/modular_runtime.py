"""可选本地运行辅助；显式配置、进程身份核验和安全停机，不管理外部数据服务。"""
...
# 确定 runner 管理的绝对工作目录；无显式值时采用此脚本所属工作树，不接受不存在的目录。
...
# 限制契约中的文件路径位于工作树内，并拒绝把 .env 当作可隐式读取的仓库文件。
...
# 读取经过工作树边界校验的普通 JSON；缺失文件或非对象结构按安全配置错误处理。
...
# 加载当前 infrastructure 声明并核验契约版本，历史 ops JSON 不再参与启动。
def _load_contract(root: Path) -> dict[str, Any]:
    contract = _read_json(root, str(CONTRACT_RELATIVE))
    if contract.get("schema_version") != "iteration7-modular-runtime-v3":
...
# 复用产品配置解析器读取明确指定的兼容配置文件，字段仍受允许清单约束。
...
def _parse_model_env_lines(lines: Iterable[str]) -> dict[str, str]:
    """只提取明确允许的模型字段；不展开变量、不执行表达式，也不吸收无关配置。"""
...
# 校验显式模型文件及祖先后读取允许字段，拒绝链接或目录伪装成配置文件。
...
# 合并模型字段前拒绝未知键和与主配置冲突的值，避免静默改变模型选择。
...
# 读取兼容配置、核验字段，再合并显式模型文件；缺少必要路径时不自动发现 .env。
...
# 由统一 schema 校验 runner 输入，保持 CLI 与产品进程对未知字段和秘密冲突的处理一致。
...
# 按模块角色构造隔离的子进程环境，不把父进程所有变量直接传给产品进程。
def _safe_child_env(
...
# 校验 URL 的回环主机与路径约束，避免辅助探测被配置到非本机服务。
...
# 把校验后的端口投影到当前运行声明，验证端口互异并保持模块入口来源。
...
# 启动或探测前检查必需配置、回环端点和观测数据边界，不通过改值绕过错误。
...
# 校验前端允许字段和三个公开回环 URL，秘密字段不能经前端配置进入浏览器。
...
def _probe_database(database_url: str) -> dict[str, Any]:
    """只执行连接与表名元数据 SELECT 探测，汇报缺失表名并关闭临时引擎，不查询业务行。"""
    try:
...
# 发送 Redis PING 后关闭探测客户端，不读取队列、登记 worker 或写入业务键。
...
# 在配置校验后解析所需秘密并逐项探测外部依赖；失败保持明确状态，不启动 MySQL/Redis。
...
# 将已筛选的状态对象稳定序列化；秘密过滤由调用方完成，此函数不是任意数据脱敏器。
def _safe_json(value: Any) -> str:
    return json.dumps(value, ensure_ascii=False, sort_keys=True, indent=2)


# 输出调用方提供的安全状态摘要，JSON 与文本模式共享状态字段，不额外输出配置正文。
...
# 按合法 run-id 定位本 runner 的临时状态目录，拒绝链接或不安全的路径关系。
...
# 先写本轮临时状态文件再原子替换，避免状态读取者看到半份进程归属记录。
...
# 只读取指定 run-id 的状态并核验结构，不扫描或认领其他运行批次。
...
# 对命令参数序列计算身份指纹，供后续核对同一 PID 是否仍属于该启动记录。
...
# 读取 Windows 进程创建时间和命令行供内部身份核验；无法取得可信身份时返回缺失。
...
# 从当前 PID 的 proc 元数据读取启动标识与命令，读取失败不推测身份。
...
# 按平台选择进程身份读取器；返回的身份仅用于归属核验，不决定端口所有权。
...
# 同时核对 PID 对应的创建时间和命令指纹；PID 复用或身份不可读时拒绝认领。
...
# 在有界时间内等待记录的 PID 消失，不通过等待逻辑终止任何进程。
...
# 终止前重新确认创建时间和命令身份；归属不明立即拒绝，不按端口或进程名杀进程。
...
# 从当前声明解析模块命令与工作目录，先确认入口和解释器可用，再交给启动流程。
...
# 以超时和受限响应读取探测 HTTP 状态；错误响应转为就绪摘要，不携带业务响应正文。
...
# 在截止时间前轮询 HTTP 探测，超时返回最后状态，不重启服务或修改端口。
...
# 结合记录中模块的进程归属与 HTTP 就绪结果计算状态，不能用可访问端口代替身份验证。
...
# 统一选择三文件或兼容显式配置模式并验证前后端契约，不搜索其他配置来源。
...
# 仅解析和校验配置，明确标记未执行外部探测，不以此宣称数据库可用。
...
# 配置通过后运行最小外部探测，返回综合就绪结果；不启动产品或外部依赖。
...
# 先核验配置、依赖和端口，再按声明顺序启动并记录归属；失败回收已确认属于本轮的进程。
def _start(root: Path, env_file: str | None, frontend_env_file: str | None, timeout: float, frontend_port: int | None = None, model_env_file: str | None = None, moonshot_model: str | None = None, *, backend_env_file: str | None = None, observability_env_file: str | None = None) -> dict[str, Any]:
    if not all((backend_env_file, frontend_env_file, observability_env_file)):
        raise RuntimeErrorBase("runtime_config:three_explicit_sources_required")
    if os.name != "nt":
        raise RuntimeErrorBase("telemetry_ipc:windows_required")
    ...
    # 使用角色隔离环境启动单个模块并记录 PID、创建时间和命令指纹，无法确认身份时不留下可认领状态。
    def launch(module: str) -> None:
        command, cwd = _module_command(root, module, contract)
        log_path = logs_root / f"{module}.log"
        with log_path.open("ab") as log_file:
            # 子进程只继承清洁的系统环境和显式路径，不转交配置值或 token。
            creation_env = configuration.isolated_environment({}, role=module)
            if module == "frontend":
                command.extend(["--env-file", str(frontend_env_file)])
            else:
                command.extend(["--repo-root", str(root)])
                role_sources = {
                    "legacy-api": ("backend", "frontend"),
                    "agent-api": ("backend", "frontend", "observability"),
                    "worker": ("backend", "observability"),
                    "observability-api": ("observability", "frontend"),
                }
                source_paths = {"backend": backend_env_file, "frontend": frontend_env_file,
                                "observability": observability_env_file}
                for source in role_sources[module]:
                    command.extend([f"--{source}-env-file", str(source_paths[source])])
...
# 依据本 run-id 的进程身份汇总存活状态，不把其他独立终端进程算作本轮成员。
...
# 在已记录工作目录和模块边界内进行就绪探测，既有状态的真实成员决定检查范围。
...
# 按依赖逆序关闭记录内且身份相符的模块；任何归属不明成员导致 ownership_unproven，不扩大停机范围。
...
# 构造辅助命令及显式配置参数，帮助输出本身不读取配置、不探测服务。
...
# 解析命令后分派配置检查、探测或生命周期操作；只输出安全状态和退出码，异常不泄露配置值。
