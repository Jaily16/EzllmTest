# 统一分派工作流、复用有效缓存并处理终态；失败、取消和截断不晋升为有效产物。
...
from ezllmtest.platform.ai.stream import get_stream_model_metadata
import ezllmtest.modules.generation.ports.artifacts as workflowArtifactService
from ezllmtest.modules.generation.application.analysis_stream import stream_analysis_operation
from ezllmtest.modules.generation.application.case_stream import stream_case_operation
from ezllmtest.modules.generation.application.stream_core import DisconnectCheck, WorkflowContext, WorkflowStreamError, ensure_connected, event, progress, usage_counters
from ezllmtest.modules.generation.domain.catalog import get_workflow_definition, list_workflow_definitions
from ezllmtest.modules.generation.domain.budget import profile_for
...
def ensure_supported_workflow(operation: str) -> None:
    """从固定目录核验 operation 可流式执行，未知操作返回稳定错误而不尝试动态装配。"""
    try:
...
def _cached_answer_text(result: Any) -> str:
    """按工作流历史产物字段查找可重放正文；非文本或没有有效字段的结果返回空文本。"""
...
async def stream_llm_workflow(
    ...
    *,
    ...
    """统一执行前置检查、revision 缓存恢复和分析/用例分派；只在完整有效结果与连接检查通过后提交持久化，失败或截断保留旧结果。"""
