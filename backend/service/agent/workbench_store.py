...
class AgentWorkbenchStore:
    ...
    def _text(value: bytes | str | None) -> str | None:
        """处理文本，并保持 `AgentWorkbenchStore` 的现有状态约束。"""
        if value is None:
            return None
    ...
    async def active_thread(self, scope_hash: str) -> str | None:
        """处理活跃线程，并保持 `AgentWorkbenchStore` 的现有状态约束。
    ...
    async def register_run(
        ...
        参数:
            `scope_hash`：沿用签名中 `str` 类型约束的输入。
            `thread_id`：Agent 运行线程 ID。
            `budget_preset`：沿用签名中 `AgentBudgetPreset | str` 类型约束的输入。
            `created_at`：沿用签名中 `datetime` 类型约束的输入。
    ...
    async def list_run_threads(
        ...
        *,
        ...
        返回:
            `tuple[tuple[str, ...], int | None]`，内容保持现有调用方契约。
    ...
    async def put_evidence(
        ...
        参数:
            `scope_hash`：沿用签名中 `str` 类型约束的输入。
            `thread_id`：Agent 运行线程 ID。
            `evidence`：沿用签名中 `AgentStepEvidence` 类型约束的输入。
    ...
    async def get_evidence(
        ...
        返回:
            `tuple[AgentStepEvidence, ...]`，内容保持现有调用方契约。
    ...
    def _timeline_keys(
        ...
        """处理时间线键，并保持 `AgentWorkbenchStore` 的现有状态约束。"""
    ...
    async def replay_timeline(
        ...
        返回:
            `tuple[AgentTimelineEvent, ...]`，内容保持现有调用方契约。
    ...
    async def accept_progress(
        ...
        参数:
            `scope_hash`：沿用签名中 `str` 类型约束的输入。
            `thread_id`：Agent 运行线程 ID。
            `step_stage`：沿用签名中 `str` 类型约束的输入。
            `bucket`：沿用签名中 `int` 类型约束的输入。
    ...
    async def timeline_bounds(
        ...
        """处理时间线边界，并保持 `AgentWorkbenchStore` 的现有状态约束。
        ...
        返回:
            `tuple[int, int] | None`，内容保持现有调用方契约。
