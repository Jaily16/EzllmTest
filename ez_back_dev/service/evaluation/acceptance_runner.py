...
from service.evaluation.acceptance_contracts import (
...
from service.evaluation.eval_runner import _environment as eval_environment
from service.evaluation.eval_runner import run_eval
from service.agent.planner import DeterministicAgentPlanner, PlannerProposal
from service.agent.coordinator import AgentRedisCoordinator, AgentRedisSettings
...
from service.agent.worker import AgentCommandWorker

ROOT = Path(__file__).resolve().parents[2]
PROJECT_ROOT = ROOT.parent
# 历史验收数据已经按职责归档；这里显式指向不可变 fixture，不能回写旧平面路径。
HISTORICAL_FIXTURE_ROOT = ROOT / "tests" / "fixtures" / "historical" / "iteration4"
FIXTURE = HISTORICAL_FIXTURE_ROOT / "iteration4_agent_acceptance_v1.json"
_PROVIDER_KEYS = (
    "ZHIPU_API_KEY",
    "DASHSCOPE_API_KEY",
    "DEEPSEEK_API_KEY",
...
async def _integrated_runtime_probe(
    ...
    with tempfile.TemporaryDirectory(prefix="ezllm-aspect8-") as temp:
        ...
        try:
            ...
            snapshot = await client.get(f"/agent/v1/projects/{project_id}/runs/{thread_id}")
            ...
            final = await client.get(f"/agent/v1/projects/{project_id}/runs/{thread_id}")
            ...
            hidden = await client.get(f"/agent/v1/projects/acceptance-alpine/runs/{thread_id}")
            events = await workbench.replay_events(runtime_scope(project_id), thread_id, 0)
            ...
            if approved_failure.status_code != 200 or not await worker.run_once(block_ms=50):
                raise RuntimeError("acceptance retryable failure was not executed")
            failed_final = await client.get(f"/agent/v1/projects/{project_id}/runs/{failed_thread}")
...
def run_acceptance(
    ...
    *,
    ...
    cases = tuple(
        case for case in dataset.cases if selected is AcceptanceSuite.ALL or case.suite is selected
    )
    results = []
    for case in cases:
    ...
    recovery_ratio = (
        sum(item.passed for item in reliability) / len(reliability) if reliability else 1.0
    )
