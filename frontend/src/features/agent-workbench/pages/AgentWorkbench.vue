...
// 工作台编排运行选择、审批和恢复；浏览器只提交受控命令，时间线不驱动工具副作用。
...
import { useAgentEvents } from "@/features/agent-workbench/composables/useAgentEvents";
import {
  cancelAgentRun,
  createAgentRun,
  decideAgentApproval,
  ...
  recoverAgentRun,
} from "@/entities/agent-run/api/agentRuns";
import type {
  AgentBudgetPreset,
  AgentCapabilities,
  AgentEvidence,
...
/** 仅将当前 Trace ID 写入剪贴板，权限失败时提示手动复制。 */
...
/** 携带当前 Trace ID 导航到观测页，详情加载由观测功能负责。 */
const openTrace = () => {
  const traceId = currentRun.value?.trace_id;
  if (!traceId) return;
  void router.push({
...
/** 优先使用能力接口允许的模型选项；列表为空时采用既有展示回退，后端仍需校验实际提交。 */
...
/** 按本地中文时间展示有效时间戳，无法解析时保留原值。 */
const formatTime = (value: string): string => {
  const date = new Date(value);
  return Number.isNaN(date.getTime()) ? value : date.toLocaleString("zh-CN");
};

/** 仅为页面展示缩进序列化已有对象，不发送或保存正文。 */
const formatJson = (value: unknown): string => JSON.stringify(value, null, 2);

/** 区分 thread 会话证据与项目产物摘要，避免把会话结果描述为长期项目保存。 */
...
/** 刷新项目运行摘要和活动 thread 标识，供创建按钮与历史选择使用。 */
...
/** 重新读取当前运行及历史；读取失败标记 thread 失效并停止事件订阅。 */
...
/** 从当前时间线最大序号续订，状态更新通过快照刷新保持与后端一致。 */
const subscribe = () => {
  if (!currentRun.value) return;
  ...
  eventStream.start({
    ...
    afterSequence,
    /** 按 sequence 去重并排序事件，再刷新运行快照；不在浏览器执行事件对应的工具。 */
    onEvent: (event) => {
...
/**
 * 停止旧订阅后读取所选 thread，保存选择并建立新连接，失败清除当前运行视图。
...
/** 要求项目身份后加载能力与历史，优先恢复活动运行，再考虑本地记住的有效 thread。 */
...
/** 拒绝空目标、重复提交或已有活动 thread；排队成功后切换到新运行。 */
...
/** 打开审批对话框，实际批准仍由明确的提交动作触发。 */
const openApprovalDialog = () => {
  approvalDialogOpen.value = true;
};

/** 等待对话框渲染完成后聚焦主按钮，支持键盘操作。 */
...
/**
 * 把决定与当前 plan_hash 一起提交，随后重新读快照并恢复订阅。
...
/** 用当前目标填充编辑草稿，不立即修改后端计划。 */
const openEditDialog = () => {
  editedGoal.value = currentRun.value?.goal || "";
  editDialogOpen.value = true;
};

/** 携带审批计划哈希提交非空目标，避免基于旧计划静默重规划。 */
...
/** 请求后端恢复并对账，再读取快照和续订事件；浏览器不重放副作用。 */
...
/** 导航到证据对应的产品页面，不在工作台内重建产物正文。 */
const goToEvidence = (path: string) => {
  void router.push(path);
};

/** 挂载后读取能力和运行历史；实际创建、审批与恢复均保留为显式用户动作。 */
onMounted(() => {
  void initialize();
});
</script>
