...
WORKSPACE_COMPONENTS = FRONTEND_ROOT / "src" / "components" / "workspace"
...
def test_llm_stream_tracks_existing_persistence_meta_without_transport_changes():
    stream = read("ez_front_dev/src/composables/useLlmStream.ts")
    ...
    assert 'meta.persistence = data.persistence === "artifact"' in stream
...
def test_result_replacement_confirmation_preserves_existing_copy_and_behavior():
    helper = read("ez_front_dev/src/ui/confirmations.ts")
...
def test_ui_test_pilot_contract_graduates_to_all_test_workspaces():
    source = read("ez_front_dev/src/components/UITest.vue")
    ...
    assert 'runWorkflow("ui_info"' in source
    assert 'runWorkflow("ui_case"' in source
    ...
    for filename in (
        ...
        workspace = read(f"ez_front_dev/src/components/{filename}")
