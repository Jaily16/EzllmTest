...
def test_shell_uses_semantic_landmarks_and_one_navigation_tree():
    source = read("ez_front_dev/src/features/workspace/MainView.vue")
...
def test_shell_preserves_routes_guards_and_navigation_state_semantics():
    ...
    for route in guarded_routes:
        assert f'index="{route}" :disabled="!isWorkflowRouteAllowed(\'{route}\')"' in normalized
    assert source.count(':disabled="!isWorkflowRouteAllowed(') == 10
    assert re.search(
        r'type NavigationState\s*=\s*"loading"\s*\|\s*"completed"\s*\|\s*"current"',
        source,
    )
    for state, label in (
        ("loading", "加载中"),
...
def test_mobile_drawer_has_keyboard_focus_and_cleanup_contracts():
    ...
    assert re.search(r"watch\(\s*\(\)\s*=>\s*route\.path", source)
