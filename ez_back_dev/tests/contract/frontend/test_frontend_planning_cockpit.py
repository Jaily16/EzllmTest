...
PLANNING_ASSETS = (
    FRONTEND_ROOT / "src" / "features" / "testing" / "assets" / "illustrations"
)
...
def test_project_analysis_bundle_uses_read_only_rest_without_implicit_stream():
    state = read("ez_front_dev/src/features/planning/state/projectAnalysis.ts")
...
def test_regeneration_confirmation_precedes_lock_and_preserves_previous_bundle():
    ...
    assert "saved.value" in plan and "ready.value" in plan
...
def test_workspace_registry_and_card_publish_eight_typed_accessible_entries():
    registry = read("ez_front_dev/src/features/testing/config/testWorkspaces.ts")
    card = read("ez_front_dev/src/features/planning/components/TestWorkspaceCard.vue")
    ...
    assert "@/features/testing/assets/illustrations/" in registry
