...
def _load_snapshot(pid: str) -> dict[str, list[str]]:
    ...
    返回:
        `dict[str, list[str]]`，内容保持现有调用方契约。

    异常:
        `ProjectSetupNotFoundError, ProjectSetupPersistenceError`：输入、状态或下游结果不满足现有约束时抛出。"""
...
def get_status(pid: str) -> ProjectSetupStatus:
    ...
    返回:
        `ProjectSetupStatus`，内容保持现有调用方契约。
...
def analyze_project_type(pid: str) -> int:
    ...
    异常:
        `ProjectSetupValidationError, ProjectSetupPersistenceError`：输入、状态或下游结果不满足现有约束时抛出。"""
    snapshot = _load_snapshot(pid)
