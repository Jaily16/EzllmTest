...
from service.agent.contracts import AgentRunState, ContextBinding, PlannedToolCall
...
def _parse_artifact_row(
    ...
    返回:
        `ContextArtifact | None`，内容保持现有调用方契约。"""
    try:
...
def _default_artifact_loader(
    ...
    参数:
        `project_id`：项目 ID。
        `source_operation`：沿用签名中 `str` 类型约束的输入。

    返回:
        `tuple[ContextArtifact, ...]`，内容保持现有调用方契约。"""
...
class AgentContextAssembler:
    ...
    def _source_operation(call: PlannedToolCall) -> str:
        """处理来源操作，并保持 `AgentContextAssembler` 的现有状态约束。
        ...
        异常:
            `ContextAssemblyError`：输入、状态或下游结果不满足现有约束时抛出。"""
    ...
    def _candidates(
        ...
        """处理candidates，并保持 `AgentContextAssembler` 的现有状态约束。
