...
from ezllmtest.platform.ai.stream import INTERMEDIATE_MAX_TOKENS
from ezllmtest.modules.generation.schemas.analysis import ApiList, IntegrationTestMenu, NonfunctionalTestMethodList, QualifiedUnitTestMenu, UnitTestMethod, UseCaseList
import ezllmtest.modules.generation.domain.prompts.text as prompt
from ezllmtest.modules.generation.domain.prompts.templates import INTEGRATION_TEST_INFO_MAP_TEMPLATE, INTEGRATION_TEST_INFO_REDUCE_TEMPLATE, INTEGRATION_TEST_INFO_STUFF_TEMPLATE, UNIT_TEST_TYPE_JSON_TEMPLATE, UNIT_TEST_UNIT_INFO_MAP_TEMPLATE, UNIT_TEST_UNIT_INFO_REDUCE_TEMPLATE, UNIT_TEST_UNIT_INFO_STUFF_TEMPLATE
from ezllmtest.modules.generation.application.stream_core import WorkflowContext, WorkflowStreamError, ensure_connected, event, parse_structured_result, progress, prompt_with_content, require_integer, require_source_revision, require_string, stream_map_reduce, stream_model_call, structured_prompt
from ezllmtest.modules.generation.domain.long_text import LongTextStrategy, strategy_for
from ezllmtest.modules.generation.domain.unit_reference import encode_unit_menu, parse_unit_reference
...
from ezllmtest.modules.knowledge.public import testdoc_text_splitter_for_acceptance, testdoc_text_splitter_for_db, testdoc_text_splitter_for_integration, testdoc_text_splitter_for_ui, testdoc_text_splitter_for_unit, testdoc_text_splitter_for_use_case
...
async def _model_value(
    ...
    *,
    ...
    """转发模型流中的事件并提取最终值，调用预算与空结果检查由共享 stream_core 执行。"""
    value = ""
    kwargs: dict[str, Any] = {}
...
async def _map_reduce_value(
    ...
    *,
    ...
    """转发 map/reduce 的进度和结果，避免调用者另建一套长文档归并流程。"""
...
async def stream_configured_analysis(
    ...
    *,
    ...
    """按 operation 的长文本策略选择直接上下文、检索或 map/reduce，结构化结果验证通过后才返回可保存值。"""
...
async def stream_nonfunctional_info(
    ...
    """提取非功能测试方法并校验结构化菜单，缺少检索资料时明确失败，不生成无依据的成功结果。"""
...
async def stream_unit_info(
    ...
    """核验具名单元引用后提取该单元信息，保持同名类或函数的限定身份，不扩大检索范围。"""
...
async def _integration_document_prompt(
    ...
    """按集成分析策略构造文档上下文，必要时分批归并或检索，并绑定来源 revision。"""
    map_prompt = ""
    reduce_prompt = ""
    stuff_prompt = ""
    documents: list[Any] = []
...
async def stream_integration_menu(
    ...
    """生成集成测试层级菜单并验证结构，已有单元信息与完整文档路径分别处理。"""
...
async def stream_analysis_operation(
    ...
    """按固定 operation 分派分析流程，未知操作抛出工作流错误，不动态导入任意函数。"""
