<template>
  ...
  <TestWorkspaceScaffold
    ...
    title="集成测试"
    description="按范围识别、集成对象分析和用例生成三个阶段恢复结果并继续工作。"
    :steps="workflowSteps"
  ...
  >
    ...
    <WorkspaceSection
      title="1. 集成测试范围"
      description="识别系统、子系统、模块和类级集成范围。"
      :busy="activeOperation === 'integration_menu' && isRunning"
    >
      <el-alert
        v-if="staleWarning('integration_menu')"
        :title="staleWarning('integration_menu')"
        type="warning"
        show-icon
        :closable="false"
      />
      ...
      <WorkflowActionBar aria-label="集成测试范围操作">
        <el-button
          v-if="!menuResult && !staleWithoutContent('integration_menu')"
          type="primary"
          :icon="Right"
          :disabled="isRunning || !canRunStep('integration_menu')"
          round
          @click="analyzeMenu(false)"
          >{{
            needsResultRecovery("integration_menu")
              ? "恢复匹配的已保存结果"
              : "开始集成测试范围分析"
          }}</el-button
        >
        <el-button
          v-if="hasSavedResult('integration_menu')"
          type="warning"
          :icon="Refresh"
          :disabled="isRunning"
          plain
          ...
          >重新生成集成测试范围</el-button
        >
      </WorkflowActionBar>
      <p v-if="needsResultRecovery('integration_menu')" class="recovery-note">
        恢复会按当前模型与输入查找已保存结果；匹配时直接恢复，不匹配时会开始新的分析。
      </p>
      <LlmWorkflowExecution
        v-if="activeOperation === 'integration_menu'"
    ...
    <el-alert
      ...
      title="正在重新生成集成测试范围"
      description="旧的对象选择与下游结果暂时隐藏；成功后采用新范围，失败或取消后恢复原结果。"
      type="warning"
      show-icon
      :closable="false"
    />
    <WorkspaceSection
      ...
      title="2. 集成对象分析"
      description="选择集成层级；需要具体对象时使用完整后端名称。"
      :busy="activeOperation === 'integration_info' && isRunning"
    >
      <el-alert
        v-if="integrationInfoWarning"
        :title="integrationInfoWarning"
        type="warning"
        show-icon
        :closable="false"
      />
      <TestFieldGroup legend="集成测试类型" field-id="integration-type" required
        ><el-radio-group
          v-model="integrationType"
          aria-required="true"
          @change="selectIntegrationType"
          ><el-radio-button
      ...
      <TestTargetSelector
        v-if="needsUnit"
        v-model="integrationUnit"
        :options="integrationUnitOptions"
        label="集成测试对象"
        field-id="integration-target"
        description="保留后端返回的完整对象名称；改变目标会使已有对象分析和用例过期。"
        placeholder="请选择集成测试对象"
        required
        @update:model-value="markIntegrationInfoStale"
      />
      <ModelSelector
      ...
      <WorkflowActionBar v-if="integrationType" aria-label="集成对象分析操作">
        <el-button
          v-if="!integrationInfoResultVisible"
          type="primary"
          :icon="Right"
          :disabled="
            isRunning || (needsUnit && !integrationUnit) || !canRunStep('integration_info')
          "
          round
          @click="analyzeIntegration(false)"
          >{{
            needsResultRecovery("integration_info") ? "恢复匹配的已保存结果" : "分析集成测试对象"
          }}</el-button
        >
        <el-button
          v-if="integrationInfoResultVisible"
          ...
          plain
          round
          @click="regenerateIntegration"
          >重新生成集成对象分析</el-button
        >
      </WorkflowActionBar>
      <p v-if="needsResultRecovery('integration_info')" class="recovery-note">
        请选择与原结果一致的类型、对象和模型以优先命中缓存；不匹配时可能执行新分析。
      </p>
      <template v-if="integrationInfoExecutionVisible">
        <LlmWorkflowExecution
          v-if="activeOperation === 'integration_info'"
          v-bind="executionProps"
          @cancel="cancel"
        />
      </template>
      <ResultContainer
        v-if="visibleIntegrationInfo"
        title="业务资料中的集成对象内容"
        :status="integrationInfoWarning ? 'stale' : 'default'"
        retention="persistent"
        ><TestResultText :text="visibleIntegrationInfo"
      /></ResultContainer>
    </WorkspaceSection>

    <WorkspaceSection
      v-if="visibleIntegrationInfo && !menuRegenerating"
      title="3. 集成测试用例"
      description="选择集成策略和输出格式后生成当前页面结果。"
      :busy="activeOperation === 'integration_case' && isRunning"
    >
      <el-alert
        v-if="caseWarning"
        :title="caseWarning"
        ...
        :closable="false"
      />
      <TestFieldGroup legend="集成测试策略" field-id="integration-strategy" required
        ><el-radio-group v-model="strategyIndex" aria-required="true" @change="markCaseStale"
          ><el-radio-button
      ...
      ></TestFieldGroup>
      <TestFieldGroup legend="测试用例输出格式" field-id="integration-output-format"
      ...
      <WorkflowActionBar aria-label="集成测试用例操作">
        <el-button
          ...
          type="primary"
          :icon="Right"
          :disabled="isRunning || strategyIndex < 0 || !canRunStep('integration_case')"
          round
      ...
      <template v-if="caseExecutionVisible">
        <LlmWorkflowExecution
          v-if="activeOperation === 'integration_case'"
          v-bind="executionProps"
          @cancel="cancel"
        />
      </template>
      <div v-if="visibleCaseResult && !menuRegenerating" class="test-results">
        <ResultContainer
          ...
          ><TestResultText :text="visibleCaseResult.integration_test_knowledge"
        /></ResultContainer>
        <ResultContainer
          title="知识库中的所选集成策略知识"
          :status="caseWarning ? 'stale' : 'default'"
          retention="session-only"
          ><TestResultText :text="visibleCaseResult.integration_strategy_knowledge"
        /></ResultContainer>
        <ResultContainer
          title="知识库中的静态黑盒测试知识"
          :status="caseWarning ? 'stale' : 'default'"
          retention="session-only"
          ><TestResultText :text="visibleCaseResult.static_blackbox_knowledge"
        /></ResultContainer>
        <ResultContainer
          title="模型生成的集成测试用例"
          :status="caseWarning ? 'stale' : 'default'"
        ...
        /></ResultContainer>
        <WorkflowActionBar aria-label="集成测试结果操作"
...
const testWorkflow = useTestWorkflow({
  ...
  steps: [
    { operation: "integration_menu", label: "集成测试范围", artifactKey: "integration_menu" },
    {
      ...
      label: "集成对象分析",
      artifactKey: "integration_info",
      dependsOn: ["integration_menu"],
      selectionFields: ["integration_type", "name"],
    },
    {
      operation: "integration_case",
      label: "集成测试用例",
      artifactKey: "integration_case",
      dependsOn: ["integration_info"],
      selectionFields: ["strategy_type", "strategy", "integration_object", "output_type"],
      persistResult: false,
    },
  ],
});
...
const menuRegenerating = computed(
  () =>
    menuRegenerationRequested.value &&
    isRunning.value &&
    activeOperation.value === "integration_menu",
);
/**
...
const availableTypes = computed(() => {
  ...
  if (
    menu.subsystem_integration_menu.subsystem_test &&
    menu.subsystem_integration_menu.subsystem_list.length
  )
    values.push(allTypes[2]);
  if (menu.module_integration_menu.module_test && menu.module_integration_menu.module_list.length)
    values.push(allTypes[3]);
  if (menu.class_integration_menu.class_test && menu.class_integration_menu.class_list.length)
    values.push(allTypes[4]);
  return values;
});
/**
 * 派生用于界面展示或请求判断的集成类型索引。
 */
const integrationTypeIndex = computed(() => allTypes.indexOf(integrationType.value));
/**
 * 派生用于界面展示或请求判断的needs单元。
 */
const needsUnit = computed(() => integrationTypeIndex.value >= 2);
/**
 * 派生用于界面展示或请求判断的集成 units。
 */
...
/**
 * 派生用于界面展示或请求判断的集成单元选项。
 */
const integrationUnitOptions = computed(() =>
  integrationUnits.value.map((value) => ({ value, label: value })),
);
/**
 * 派生用于界面展示或请求判断的当前集成 info选择条件。
 */
...
/**
 * 派生用于界面展示或请求判断的集成 info结果可见状态。
 */
const integrationInfoResultVisible = computed(() =>
  hasVisibleResultFor("integration_info", currentIntegrationInfoSelection.value),
);
/**
 * 派生用于界面展示或请求判断的可见状态集成 info。
 */
const visibleIntegrationInfo = computed(() =>
  integrationInfoResultVisible.value ? integrationInfo.value : "",
);
/**
 * 派生用于界面展示或请求判断的集成 info execution可见状态。
 */
const integrationInfoExecutionVisible = computed(
  () =>
    activeOperation.value === "integration_info" &&
    (isRunning.value ||
      selectionsMatch(
        "integration_info",
        activeSelection.value,
        currentIntegrationInfoSelection.value,
      )),
);
/**
 * 派生用于界面展示或请求判断的当前集成 object。
 */
const currentIntegrationObject = computed(() =>
  integrationTypeIndex.value === 0
    ? "业务中的整个系统"
...
const caseResultVisible = computed(() =>
  hasVisibleResultFor("integration_case", currentIntegrationCaseSelection.value),
);
/**
...
const caseExecutionVisible = computed(
  () =>
    activeOperation.value === "integration_case" &&
    (isRunning.value ||
      selectionsMatch(
        "integration_case",
        activeSelection.value,
        currentIntegrationCaseSelection.value,
      )),
);
/**
 * 派生用于界面展示或请求判断的集成 info警告。
 */
const integrationInfoWarning = computed(() =>
  integrationInfoResultVisible.value ? staleWarning("integration_info") : "",
);
/**
 * 派生用于界面展示或请求判断的用例警告。
 */
const caseWarning = computed(() =>
  caseResultVisible.value ? staleWarning("integration_case") : "",
);
/**
...
/**
 * 选择集成类型，并保持现有状态与错误处理语义。
 */
const selectIntegrationType = () => {
  integrationUnit.value = "";
  reconcileStepSelection("integration_info", currentIntegrationInfoSelection.value);
};
/**
 * 标记集成 info过期，并保持现有状态与错误处理语义。
 */
const markIntegrationInfoStale = () =>
  reconcileStepSelection("integration_info", currentIntegrationInfoSelection.value);
/**
 * 标记用例过期，并保持现有状态与错误处理语义。
 */
const markCaseStale = () =>
  reconcileStepSelection("integration_case", currentIntegrationCaseSelection.value, false);
/**
 * 分析集成，并保持现有状态与错误处理语义。
 *
 * @param regenerate 沿用当前 TypeScript 类型约束的输入。
...
const analyzeIntegration = async (regenerate = false): Promise<boolean> => {
  ...
  if (needsUnit.value && !integrationUnit.value) {
    ElMessage.warning("请先选择要进行集成测试的对象");
    return false;
  }
  const succeeded = await runWorkflow(
    "integration_info",
...
const generateCases = async (regenerate = false): Promise<boolean> => {
  ...
  if (!visibleIntegrationInfo.value) {
    ElMessage.warning("请先分析当前选择的集成对象");
    return false;
  }
  const succeeded = await runWorkflow(
    "integration_case",
    llm.value,
    {
      ...
      integration_object: currentIntegrationObject.value,
      integration_object_info: visibleIntegrationInfo.value,
      output_type: outputFormat.value,
    },
    {
...
const regenerateMenu = async () => {
  ...
  try {
    return await regenerateStep("integration_menu", () => analyzeMenu(true));
  } finally {
    menuRegenerationRequested.value = false;
  }
};
/**
 * 重新生成集成，并保持现有状态与错误处理语义。
 */
...
const restoreWorkflow = async () => {
  ...
  if (typeof caseSelection.strategy_type === "number")
    strategyIndex.value = caseSelection.strategy_type;
