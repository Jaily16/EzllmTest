# 从已登记资料恢复并确认准备状态，不在资料确认阶段调用模型。
"""Recoverable, revision-aware project setup without model calls."""

from __future__ import annotations
from ezllmtest.modules.projects.schemas.setup import DocumentGroup, ProjectSetupError, ProjectSetupNotFoundError, ProjectSetupPersistenceError, ProjectSetupStatus, ProjectSetupValidationError, SetupStage
...
from ezllmtest.modules.generation.public import WorkflowArtifactRecord
from ezllmtest.modules.generation.public import get_fresh_artifact, invalidate_project_artifacts, save_artifact
from ezllmtest.modules.projects.application.revision import artifact_input_hash, compute_project_source_revision
...
from ezllmtest.modules.knowledge.public import invalidate_project_indexes
...
def _document_files(
    ...
    """取得当前项目已登记资料的受控文件集合，不递归搜寻任意工作树内容。"""
    return {
...
def _stage_for_counts(counts: dict[DocumentGroup, int]) -> SetupStage:
    """按各资料组计数推导上传阶段，三个组齐全才进入 documents_ready。"""
...
def _load_snapshot(pid: str) -> dict[str, list[str]]:
    """加载当前项目的准备快照，按 revision 判断是否仍可恢复。"""
...
def get_status(pid: str) -> ProjectSetupStatus:
    """依据当前登记资料和已存准备记录推导状态，不通过查询自动调用模型。"""
...
def _count_document_tokens(paths: list[str]) -> int:
    """离线统计当前项目资料 token，为准备和后续预算提供估计，不调用 embedding。"""
    total = 0
    for path in paths:
...
def analyze_project_type(pid: str) -> int:
    """依据已登记文档结构和内容完成项目类型准备分析，保持可恢复状态记录。"""
    snapshot = _load_snapshot(pid)
...
def finalize(pid: str) -> ProjectSetupStatus:
    """核验必需资料与来源 revision 后完成准备登记，条件不足时不开放下游生成。"""
    status = get_status(pid)
