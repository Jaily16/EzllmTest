# 执行器在批准和项目隔离边界内调用既有业务服务，结果转为受限证据。
...
import ezllmtest.modules.projects.public as projectSetupService
import ezllmtest.modules.projects.public as projectWorkflowStatusService
from ezllmtest.modules.agent.domain.contracts import AgentRunState, AgentRunStatus, ApprovalBinding, TrustedProjectScope, approval_is_valid, redact_sensitive_text
from ezllmtest.modules.agent.runtime.memory.context import context_payload_fields
from ezllmtest.modules.knowledge.public import use_agent_retrieval
from ezllmtest.modules.agent.runtime.tools.registry import DEFAULT_TOOL_REGISTRY, RegisteredToolDefinition, ToolRegistry
from ezllmtest.modules.agent.runtime.tools.schemas import ToolExecutionError, ToolExecutionResult, ToolExecutionStatus, ToolProgress, ToolUsageSummary, WorkflowToolInput
from ezllmtest.modules.generation.public import TestPlanStreamError, get_project_analysis_status, stream_test_plan
from ezllmtest.modules.generation.public import WorkflowStreamError
from ezllmtest.modules.generation.public import stream_llm_workflow
...
def _default_bindings() -> ToolServiceBindings:
    """从各业务域 public 接口构造执行绑定，不让工具执行器直接依赖其他域仓储。"""
...
def _safe_error(
    ...
    *,
    ...
    """构造稳定工具错误对象，公开消息不携带底层异常、秘密或业务正文。"""
...
# 工具执行器只接受 registry 已声明的参数和项目上下文，审批与幂等检查不能绕过。
class AgentToolExecutor:
    def __init__(
        ...
        *,
        ...
        """绑定工具注册表与业务服务接口；未显式注入时使用默认服务绑定，不在此执行工具。"""
    ...
    async def execute(
        ...
        """从固定工具目录取得定义，核验可信上下文，再分流只读操作或需授权的工作流执行。"""
        try:
    ...
    async def _execute_read(
        ...
        """调用项目只读能力并组织安全结果；只读工具不得变成隐式生成入口。"""
        service = {
    ...
    def _approval_error(
        ...
        """核验工作流调用的批准与当前上下文是否匹配，缺失或过期时返回明确审批错误。"""
    ...
    async def _cancel_requested(
        ...
        """读取当前取消检查器的结果，工具执行在关键边界据此停止后续动作。"""
    ...
    async def _execute_workflow(
        ...
        """审批和输入通过后调用既有工作流流接口，使用断连回调传播取消并保持结果保留边界。"""
        async def disconnected() -> bool:
            """将 Agent 取消状态映射到工作流断连检查，使下游在付费或保存前停止。"""
    ...
    async def _consume_stream(
        ...
        """消费进度、结果和用量事件并形成工具结果；错误或截断不能冒充成功保存。"""
