<template>
  ...
  >
    ...
    >
      ...
      <template #tip>
        <p class="document-group__tip">
          待上传文件只保留在当前浏览器页面；刷新后需要重新选择。
        </p>
      </template>
    </el-upload>
  </section>
...
import type { SetupStepState } from "@/state/projectSetup";
...
.document-group[data-state="running"] { border-color: var(--ez-color-brand-300); }
.document-group[data-state="completed"] { border-color: var(--ez-color-success); }
.document-group[data-state="failed"] { border-color: var(--ez-color-danger); }
...
.document-group__stored li { overflow-wrap: anywhere; }
.document-group__progress {
  background: var(--ez-color-brand-50);
  color: var(--ez-color-brand-700);
}
...
@media (max-width: 767px) {
  .document-group { padding: var(--ez-space-4); }
  .document-group__header {
    align-items: flex-start;
